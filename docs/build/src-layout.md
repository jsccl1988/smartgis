...
| sdb | `src/sdb/{feature,layer,map,crs,datasource/<driver>}` | GIS model; GDAL/OGR-extendable. |
| render | `src/render/` + `render/rhi` | Unified 2D+3D backends. **Paint runs in `--type=gpu`**, not in browser. |
...
SMF already opens some formats via `//third_party/gdal_sdk`. `sdb/datasource/gdal` is the register seam for a future OGR driver; it is not a full OGR feature model yet.
...
| Core data model | `sdb/{feature,layer,map}` | One DLL (`SmtGisCore` = `//src/sdb/map:gis`). | yes (`gis`) |
| Datasource | `sdb/datasource/{mgr,ado,mem,smf,ws,gdal}` | Provider drivers. `src/ado` stays SmtAdoCore. | yes (`//src/sdb:datasource` + `ado`) |
...
| Render | `render/` + children | 2D backends + 3D engines (`render3d`, `scene3d`, `model3d`, `terrain`, `pointcloud`). `d3d` unwired. `skia` opt-in stub. | yes (`render_all`, not `d3d` / not `skia`) |
...
| Tool | `tool/` + `tool/group` | Interactive tools (not UI chrome). | `tool` only (`group` needs UI) |
...
| `Smt3DBaseLib` | `scene3d` | `render/scene3d` | `scene3d` | `Smt3DBaseLib` |
| `Smt3DMdLib` | `model3d` | `render/model3d` | `model3d` | `Smt3DMdLib` |
...
| --- | --- | --- | --- |
| New (`content`, `gpu`, `app/{views,webview2,winui}`, `ui/views`, `render/skia`) | `snake_case` | `.cc` / `.h` | `"content/public/map_view.h"`, `"ui/views/view.h"`, `"gpu/gpu.h"`, `"render/skia/canvas.h"` (`//src` on the include path) |
