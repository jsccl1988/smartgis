...
| Leftover MFC exe | `SmartGis.exe` until parity | `src/app/` leftover | — |
| Legacy chrome | MFC + BCG / `src/ui` (retire after parity) | `src/ui/{gui,mfc_ex,xview,xcatalog,xambox,chart}` | `Smt_*` |
...
`build.bat app` keeps MFC Feature Pack until it is retired. `build.bat views` builds the destination chrome.
...
src/app/                         leftover MFC SmartGis.exe only

src/ui/{gui,mfc_ex,xview,        LEGACY chrome + map CView (until parity)
