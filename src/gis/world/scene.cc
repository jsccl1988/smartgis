...
void World::attach_map(const gis::SmtMap* map) {
  if (!map) {
    return;
  }
  ...
  nodes_.swap(kept);
  gis::SmtMap* walk = const_cast<gis::SmtMap*>(map);
  ...
  for (int i = 0; i < count; ++i) {
    ...
    const gis::SmtLayer* layer = map->GetLeftoverLayer(i);
    if (!layer) {
      continue;
    }
    base::Envelope env;
...
Node* World::attach_raster_layer(const gis::SmtRasterLayer* layer) {
  if (!layer) {
    return nullptr;
  }
  base::fRect rect;
...
Node* World::attach_tile_layer(const gis::SmtTileLayer* layer) {
  if (!layer) {
    return nullptr;
  }
  base::Envelope env;
...
Node* World::attach_model(const gis::ModelAsset* asset,
                          const char* name) {
  if (!asset) {
    return nullptr;
  }
  ...
  double max_y = 0;
  double max_z = 0;
  if (!gis::model_aabb(*asset, &min_x, &min_y, &min_z, &max_x, &max_y,
                              &max_z)) {
    return nullptr;
  }
...
Node* World::attach_tileset(const gis::Tileset* tileset,
                            const char* name) {
  if (!tileset) {
    return nullptr;
  }
  const gis::Tile& root = tileset->root;
