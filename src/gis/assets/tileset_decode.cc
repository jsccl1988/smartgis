...
bool skip_b3dm(const unsigned char* data, size_t size,
  ...
  size_t offset = 28;
  offset +=
      static_cast<size_t>(feature_json) + feature_bin + batch_json + batch_bin;
  if (offset >= size) {
    offset = 28;
  }
  if (offset >= size) {
...
bool copy_accessor_floats(const tinygltf::Model& model,
  ...
  const int32_t comps =
      tinygltf::GetNumComponentsInType(static_cast<uint32_t>(acc.type));
  if (elem <= 0 || comps <= 0) {
    return false;
  }
  const size_t stride =
