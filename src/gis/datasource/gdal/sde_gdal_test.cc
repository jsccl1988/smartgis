...
using gis::SmtFtAnno;
using gis::SmtFtCurve;
using gis::SmtFtDot;
using gis::SmtFtGrid;
using gis::SmtFtSurface;
using gis::SmtFtTin;
using gis::SmtFtUnknown;
using gis::SmtRasterLayer;
...
void run_sdbd_driver_tests() {
  ...
  gis::datasource::SdbdHandler handler;
  int status = 0;
  std::string body;
  handler.handle("GET", "/sdbd/api/v1/health", "", &status, &body);
  expect(
      status == 200 && body.find("\"service\":\"sdbd\"") != std::string::npos,
      "sdbd health");
  ...
  expect(status == 200 && body.find("point") != std::string::npos,
         "sdbd open layer");
  handler.handle(
      "POST", "/sdbd/api/v1/recordset/append",
  ...
  expect(status == 200 && body.find("POINT") != std::string::npos,
         "sdbd query point");
  handler.handle("POST", "/sdbd/api/v1/recordset/open", "{\"layer\":\"pts\"}",
                 &status, &body);
  ...
  if (!handle.empty()) {
    const std::string fetch = std::string("{\"handle\":\"") + handle +
                              "\",\"offset\":0,\"limit\":10}";
    handler.handle("POST", "/sdbd/api/v1/recordset/fetch", fetch, &status,
                   &body);
    expect(status == 200 && body.find("POINT") != std::string::npos,
           "sdbd recordset/fetch");
    const std::string close = std::string("{\"handle\":\"") + handle + "\"}";
...
int main() {
  ...
  if (mem) {
    ...
    if (ds) {
      ...
      if (lyr) {
        ...
        expect(gis::datasource::encode_smt_geometry(&smt_pt, &ogr, SmtFtAnno),
               "smt geom->ogr anno");
        ...
        OGRGeometry* back_g =
            gis::datasource::decode_ogr_geometry(&ogr, SmtFtAnno);
        ...
        expect(gis::datasource::infer_feature_type(&ogr, SmtFtUnknown) ==
                   SmtFtAnno,
               "nonempty anno is SmtFtAnno");
        ogr.SetField("anno", "");
        expect(
            gis::datasource::infer_feature_type(&ogr, SmtFtUnknown) == SmtFtDot,
      ...
      if (multi_lyr) {
        ...
        mpl.SetGeometry(&mp);
        OGRGeometry* mp_back =
            gis::datasource::decode_ogr_geometry(&mpl, SmtFtDot);
        ...
        OGRGeometry* mls_back =
            gis::datasource::decode_ogr_geometry(&mls, SmtFtCurve);
        ...
        OGRGeometry* mpg_back =
            gis::datasource::decode_ogr_geometry(&mpgf, SmtFtSurface);
  ...
  std::string path = gis::datasource::make_gdal_open_target(info);
  const std::string sdbd_target = gis::datasource::make_sdbd_open_target(info);
  ...
  if (!sdbd_ds) {
    ...
    if (lyr) {
      ...
      expect(gis::datasource::encode_smt_geometry(&smt_pt, &feat, SmtFtDot),
             "encode point");
      ...
      if (lyr2) {
        ...
        OGRGeometry* g =
            got ? gis::datasource::decode_ogr_geometry(got, SmtFtDot) : nullptr;
    ...
    OGRLayer* lines =
        sdbd_ds ? sdbd_ds->create_sdbd_layer("lines", SmtFtCurve) : nullptr;
    ...
    if (lines) {
      ...
      expect(gis::datasource::encode_smt_geometry(line, &feat, SmtFtCurve),
             "encode line");
      ...
      if (back) {
        ...
        OGRGeometry* g =
            got ? gis::datasource::decode_ogr_geometry(got, SmtFtCurve)
                : nullptr;
    ...
    OGRLayer* polys =
        sdbd_ds ? sdbd_ds->create_sdbd_layer("polys", SmtFtSurface) : nullptr;
    ...
    if (polys) {
      ...
      expect(gis::datasource::encode_smt_geometry(poly, &feat, SmtFtSurface),
             "encode poly");
      ...
      if (back) {
        ...
        OGRFeature* got = back->GetNextFeature();
        OGRGeometry* g =
            got ? gis::datasource::decode_ogr_geometry(got, SmtFtSurface)
                : nullptr;
    ...
    OGRLayer* annos =
        sdbd_ds ? sdbd_ds->create_sdbd_layer("annos", SmtFtAnno) : nullptr;
    expect(annos != nullptr, "create annos");
    if (annos) {
      OGRPoint anno_pt(2.0, 3.0);
      OGRFeature feat(annos->GetLayerDefn());
      expect(gis::datasource::encode_smt_geometry(&anno_pt, &feat, SmtFtAnno),
             "encode anno");
    ...
    OGRLayer* tins =
        sdbd_ds ? sdbd_ds->create_sdbd_layer("tins", SmtFtTin) : nullptr;
    ...
    if (tins) {
      ...
      expect(gis::datasource::encode_smt_geometry(&tin, &feat, SmtFtTin),
             "encode tin");
      ...
      if (back) {
        back->ResetReading();
        OGRFeature* got = back->GetNextFeature();
        SmtTin* gt = got ? gis::datasource::decode_smt_tin(got) : nullptr;
        expect(
            gt && (gt->get_triangle_count() >= 1 || gt->get_point_count() >= 3),
            "tin persist");
        delete gt;
        OGRFeature::DestroyFeature(got);
      }
    ...
    OGRLayer* grids =
        sdbd_ds ? sdbd_ds->create_sdbd_layer("grids", SmtFtGrid) : nullptr;
    ...
    if (grids) {
      ...
      expect(gis::datasource::encode_smt_geometry(&grid, &feat, SmtFtGrid),
             "encode grid");
      ...
      if (back) {
        back->ResetReading();
        OGRFeature* got = back->GetNextFeature();
        expect(got && gis::datasource::infer_feature_type(got, SmtFtGrid) ==
                          SmtFtGrid,
               "grid type");
        OGRFeature::DestroyFeature(got);
      }
  ...
  expect(gis::datasource::make_sdbd_open_target(ainfo).empty(),
         "ACCESS has no SDBD target");
  expect(gis::datasource::open_sdbd_dataset(ainfo) == nullptr,
         "ACCESS Open false");
  ...
  if (mgr) {
    ...
    gis::SmtRasterLayer* mem_ras = gis::DataSourceMgr::create_mem_ras_layer();
    expect(mem_ras != nullptr, "CreateMemRasLayer");
    if (mem_ras) {
      expect(dynamic_cast<gis::datasource::OgrRasterLayer*>(mem_ras) != nullptr,
  ...
  if (pg_dsn && pg_dsn[0]) {
    ...
    std::snprintf(pgi.db.szService, sizeof(pgi.db.szService), "%s:%s", host,
                  port);
    GDALDataset* pgds = gis::datasource::open_sdbd_dataset(pgi);
