...
GIS_EXPORT std::vector<const StyleLayer*> select_layers(
    const StyleDocument& doc, double zoom, const std::string& source_layer);

// First matching layer → ResolvedPaint (with optional symbol lookup).
GIS_EXPORT bool resolve(const StyleDocument& doc, const SymbolLibrary* library,
                        const AttrMap& attrs, double zoom,
                        const std::string& source_layer, ResolvedPaint* out);
