...
SmtIAToolManager::SmtIAToolManager(void) { m_pActiveIATool = NULL; }

SmtIAToolManager::~SmtIAToolManager(void) { RemoveAllIATool(); }

long SmtIAToolManager::notify(SmtIATool *pIATool, long lMsg,
...
SmtIATool *SmtIAToolManager::GetIATool(int index) {
  if (index < 0 || index >= m_vIAToolPtrs.size()) return NULL;
...
long SmtIAToolManager::RegisterIAToolMsg(SmtIATool *pIATool) {
  ...
  m_cslock.lock();
#endif
  if (pIATool == NULL) return SMT_ERR_INVALID_PARAM;
...
long SmtIAToolManager::UnRegisterIAToolMsg(SmtIATool *pIATool) {
  ...
  while (iter != vMsgs.end()) {
    if ((mapIter = m_mapMsgToIATools.find((*iter))) !=
        m_mapMsgToIATools.end()) {
