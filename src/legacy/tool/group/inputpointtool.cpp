...
SmtInputPointTool::SmtInputPointTool()
    : m_pGeom(NULL), m_appendType(PT_DOT), m_fAngle(0.) {
...
int SmtInputPointTool::Init(LPRENDERDEVICE pMrdRenderDevice,
...
int SmtInputPointTool::AuxDraw() { return SMT_ERR_NONE; }
...
void SmtInputPointTool::OnSetPointType(void) {
  // Type only - callers (e.g. plugin) SetActive when they need exclusive IA.
}
