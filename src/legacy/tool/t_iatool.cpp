...
SmtIATool::~SmtIATool() { UnRegister(); }
...
bool SmtIATool::SetDelegateSrc(SmtIATool *pDeleSrcTool) {
  if (m_pDelegateSrc) return false;
...
bool SmtIATool::BeginDelegate(SmtIATool *pDelegateTag) {
  if (m_pDelegateTag) return false;
...
int SmtIATool::LButtonDown(uint nFlags, lPoint point) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->LButtonDown(nFlags, point);

  return SMT_ERR_NONE;
}

int SmtIATool::LButtonUp(uint nFlags, lPoint point) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->LButtonUp(nFlags, point);
...
int SmtIATool::RButtonDown(uint nFlags, lPoint point) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->RButtonDown(nFlags, point);

  return SMT_ERR_NONE;
}

int SmtIATool::RButtonUp(uint nFlags, lPoint point) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->RButtonUp(nFlags, point);
...
int SmtIATool::MouseMove(uint nFlags, lPoint point) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->MouseMove(nFlags, point);
...
inline bool SmtIATool::IsOperDone(void) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->IsOperDone();

  return m_bDone;
}

inline void SmtIATool::SetOperDone(bool bDone) {
  if (m_pDelegateTag != NULL) m_pDelegateTag->SetOperDone(bDone);

  m_bDone = bDone;
}

inline bool SmtIATool::IsEnableContexMenu(void) {
  if (m_pDelegateTag != NULL) return m_pDelegateTag->IsEnableContexMenu();

  return m_bEnableContexMenu;
}

inline void SmtIATool::SetEnableContexMenu(bool bEnable) {
  if (m_pDelegateTag != NULL) m_pDelegateTag->SetEnableContexMenu(bEnable);
...
int SmtIATool::Init(HWND hWnd, pfnToolCallBack pfnCallBack, void *pToFollow) {
  if (NULL == hWnd) return SMT_ERR_INVALID_PARAM;
...
int SmtIATool::EndIA(long nMsg, SmtListenerMsg &param) {
  ...
  if (NULL != m_pfnCallBack) (*m_pfnCallBack)(nMsg, param);

  if (NULL != m_pDelegateSrc) m_pDelegateSrc->notify(nMsg, param);
