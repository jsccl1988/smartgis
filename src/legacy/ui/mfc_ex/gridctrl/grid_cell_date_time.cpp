///////////////////////////////////////////////////////////////////////////
//
// grid_cell_date_time.cpp: implementation of the CGridCellDateTime class.
//
...
#include "stdafx.h"
#include "legacy/ui/mfc_ex/gridctrl/grid_cell_date_time.h"
...
BOOL CGridCellDateTime::Edit(int nRow, int nCol, CRect rect, CPoint /* point */,
  ...
  m_pEditWnd =
      new CInPlaceDateTime(GetGrid(), rect, m_dwStyle | DTS_UPDOWN, nID, nRow,
                           nCol, GetTextClr(), GetBackClr(), GetTime(), nChar);
  return TRUE;
}

CWnd* CGridCellDateTime::GetEditWnd() const { return m_pEditWnd; }
...
#ifdef _WIN32_WCE
    CString strTemp;
    strTemp.Format(_T("%02d:%02d:%02d"), m_cTime.GetHour(), m_cTime.GetMinute(),
                   m_cTime.GetSecond());
    SetText(strTemp);
#else
...
#ifdef _WIN32_WCE
    CString strTemp;
    strTemp.Format(_T("%02d/%02d/%02d"), m_cTime.GetMonth(), m_cTime.GetDay(),
                   m_cTime.GetYear());
    SetText(strTemp);
#else
...
UINT CInPlaceDateTime::OnGetDlgCode() { return DLGC_WANTALLKEYS; }
