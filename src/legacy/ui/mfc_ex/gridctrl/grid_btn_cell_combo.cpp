...
*****************************************************************************/
#include "stdafx.h"
#include "legacy/ui/mfc_ex/gridctrl/grid_btn_cell_combo.h"

#include "legacy/ui/mfc_ex/gridctrl/in_place_list.h"
...
CGridBtnCellCombo::CGridBtnCellCombo() : CGridBtnCell() {}
...
void CGridBtnCellCombo::OnClick(CPoint PointCellRelative) {
  ...
  if (iCtlHit >= 0) {
    ...
    bHitScrollDown = (uiType == DFC_SCROLL) && uiState & DFCS_SCROLLDOWN;
  }
  if (bHitScrollDown) {
    ...
    CRect rect;
    if (!pGrid->GetCellRect(m_iRow, m_iCol, rect)) return;

    SendMessageToParent(m_iRow, m_iCol, GVN_BEGINLABELEDIT);

    Edit(m_iRow, m_iCol, rect, PointCellRelative, IDC_INPLACE_CONTROL,
         VK_LBUTTON);
    return;
  }
...
*****************************************************************************/
BOOL CGridBtnCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint point,
                             UINT nID, UINT nChar) {
  ...
  if (iCtlHit >= 0) {
    ...
    bShowDropDownNow = (uiType == DFC_SCROLL) && uiState & DFCS_SCROLLDOWN;
  }
  BOOL bHitNonComboButton = (iCtlHit >= 0) && !bShowDropDownNow;

  if (HasCellText() && !bHitNonComboButton) {
    m_ucEditing = TRUE;

    // InPlaceList auto-deletes itself
    CGridCtrl* pGrid = GetGrid();
    CInPlaceList* pInPlaceList = new CInPlaceList(
        pGrid, rect, m_dwComboStyle, nID, nRow, nCol, GetTextClr(),
        GetBackClr(), m_StringArrayCombo, GetText(), nChar);

    if (bShowDropDownNow) {
      if (m_dwComboStyle & CBS_DROPDOWN || m_dwComboStyle & CBS_DROPDOWNLIST) {
...
void CGridBtnCellCombo::EndEdit() {
  ...
  if (pWnd) ((CInPlaceList*)pWnd)->EndEdit();
}
...
void CGridBtnCellCombo::OnEndEdit() {
  ...
  for (int i1 = 0; i1 < iCtlNbr; i1++) {
    ...
    BOOL bIsScrollDown = (uiType == DFC_SCROLL) && uiState & DFCS_SCROLLDOWN;
    BOOL bIsPushButton = (uiType == DFC_BUTTON) && uiState & DFCS_BUTTONPUSH;

    if (bIsScrollDown || bIsPushButton) {
      ClickedCellCtl(
