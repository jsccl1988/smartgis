...
// Feature Pack already defines CMemDC; keep Keith Rule's helper under a
// distinct name so GridCtrl double-buffering still compiles.
class CGridMemDC : public CDC {
 public:
  ...
  CGridMemDC(CDC* pDC) : CDC() {
    ...
    ((_WIN32_WCE > 201) && !defined(_WIN32_WCE_NO_PRINTING))
