// grid_cell.cpp : implementation file
//
...
// CGridCellBase <kenbertelson@hotmail.com> C Maunder     - 17 Jun 2000 - Font
// handling optimsed, Added CGridDefaultCell
//
/////////////////////////////////////////////////////////////////////////////
...
CGridCell::~CGridCell() { delete m_plfFont; }
...
void CGridCell::SetFont(const LOGFONT* plf) {
  if (plf == NULL) {
    ...
    if (!m_plfFont) m_plfFont = new LOGFONT;
    if (m_plfFont) memcpy(m_plfFont, plf, sizeof(LOGFONT));
  }
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */,
                     UINT nID, UINT nChar) {
  if (m_bEditing) {
    if (m_pEditWnd) m_pEditWnd->SendMessage(WM_CHAR, nChar);
  } else {
    ...
    m_pEditWnd = new CInPlaceEdit(pGrid, rect, dwStyle, nID, nRow, nCol,
                                  GetText(), nChar);
  }
  return TRUE;
}
...
void CGridCell::EndEdit() {
  if (m_pEditWnd) ((CInPlaceEdit*)m_pEditWnd)->EndEdit();
}
...
CGridDefaultCell::~CGridDefaultCell() { m_Font.DeleteObject(); }
