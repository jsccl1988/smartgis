////////////////////////////////////////////////////////////////////////////
// title_tip.cpp : implementation file
//
...
BOOL CTitleTip::Create(CWnd* pParentWnd) {
  ...
  m_bCreated =
      CreateEx(dwExStyle, TITLETIP_CLASSNAME, NULL, dwStyle, CW_USEDEFAULT,
...
void CTitleTip::Show(CRect rectTitle, LPCTSTR lpszTitleText, int xoffset /*=0*/,
  ...
  if (!IsWindow(m_hWnd)) Create(m_pParentWnd);
...
BOOL CTitleTip::PreTranslateMessage(MSG* pMsg) {
  ...
  switch (pMsg->message) {
    ...
    case WM_MBUTTONDOWN: {
      ...
      if (!pWnd) return CWnd::PreTranslateMessage(pMsg);

      if (pWnd->GetSafeHwnd() == GetSafeHwnd()) pWnd = m_pParentWnd;
