// in_place_edit.cpp : implementation file
//
...
CInPlaceEdit::CInPlaceEdit(CWnd* pParent, CRect& rect, DWORD dwStyle, UINT nID,
  ...
  m_nColumn = nColumn;
  m_nLastChar = 0;
  m_bExitOnArrows = (nFirstChar != VK_LBUTTON);  // If mouse click brought us
  ...
  // being lost SenMessage changed to PostMessage (John Lagerquist)
  if (nFirstChar < 0x80)
...
UINT CInPlaceEdit::OnGetDlgCode() { return DLGC_WANTALLKEYS; }
...
void CInPlaceEdit::EndEdit() {
  ...
  if (pOwner) pOwner->SendMessage(WM_NOTIFY, GetDlgCtrlID(), (LPARAM)&dispinfo);

  // Close this window (PostNcDestroy will delete this)
  if (IsWindow(GetSafeHwnd())) SendMessage(WM_CLOSE, 0, 0);
