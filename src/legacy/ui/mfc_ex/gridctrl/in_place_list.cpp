// in_place_list.cpp : implementation file
//
...
//              Roelf Werkman <rdw@inn.nl>. Added nID to the constructor param
//              list.
...
void CComboEdit::OnKillFocus(CWnd* pNewWnd) {
  ...
  CInPlaceList* pOwner =
      (CInPlaceList*)GetOwner();  // This MUST be a CInPlaceList
  if (pOwner) pOwner->EndEdit();
}

void CComboEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  if ((nChar == VK_PRIOR || nChar == VK_NEXT || nChar == VK_DOWN ||
       nChar == VK_UP || nChar == VK_RIGHT || nChar == VK_LEFT) &&
...
CInPlaceList::CInPlaceList(CWnd* pParent, CRect& rect, DWORD dwStyle, UINT nID,
                           int nRow, int nColumn, COLORREF crFore,
                           COLORREF crBack, CStringArray& Items,
  ...
  m_nLastChar = 0;
  m_bExitOnArrows =
      FALSE;  //(nFirstChar != VK_LBUTTON);	// If mouse click brought us here,
  ...
  // Add the strings
  for (int i = 0; i < Items.GetSize(); i++) AddString(Items[i]);
  ...
  */
  ...
  if (::IsWindow(m_hWnd) && SelectString(-1, m_sInitText) == CB_ERR)
    SetWindowText(
        m_sInitText);  // No text selected, so restore what was there before
...
void CInPlaceList::EndEdit() {
  CString str;
  if (::IsWindow(m_hWnd)) GetWindowText(str);
  ...
  if (::IsWindow(m_hWnd)) PostMessage(WM_CLOSE, 0, 0);
}
...
UINT CInPlaceList::OnGetDlgCode() { return DLGC_WANTALLKEYS; }

void CInPlaceList::OnDropdown() { SetDroppedWidth(GetCorrectDropWidth()); }

void CInPlaceList::OnKillFocus(CWnd* pNewWnd) {
  CComboBox::OnKillFocus(pNewWnd);

  if (GetSafeHwnd() == pNewWnd->GetSafeHwnd()) return;

  // Only end editing on change of focus if we're using the CBS_DROPDOWNLIST
  // style
  if ((GetStyle() & CBS_DROPDOWNLIST) == CBS_DROPDOWNLIST) EndEdit();
}
...
void CInPlaceList::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) {
  if (nChar == VK_ESCAPE) SetWindowText(m_sInitText);  // restore previous text
