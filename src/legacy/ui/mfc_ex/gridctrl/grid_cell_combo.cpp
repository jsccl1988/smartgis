// grid_cell_combo.cpp : implementation file
//
...
CGridCellCombo::CGridCellCombo() : CGridCell() {
  SetStyle(
      CBS_DROPDOWN);  // CBS_DROPDOWN, CBS_DROPDOWNLIST, CBS_SIMPLE, CBS_SORT
}
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */,
  ...
  m_pEditWnd =
      new CInPlaceList(GetGrid(), rect, GetStyle(), nID, nRow, nCol,
...
void CGridCellCombo::EndEdit() {
  if (m_pEditWnd) ((CInPlaceList*)m_pEditWnd)->EndEdit();
}
...
void CGridCellCombo::SetOptions(CStringArray& ar) {
  m_Strings.RemoveAll();
  for (int i = 0; i < ar.GetSize(); i++) m_Strings.Add(ar[i]);
}