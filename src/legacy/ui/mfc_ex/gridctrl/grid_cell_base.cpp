// grid_cell_base.cpp : implementation file
//
...
// CGridCellBase C Maunder     - 19 May 2000 - Fixed sort arrow drawing (Ivan
// Ilinov) C Maunder     - 29 Aug 2000 - operator= checks for NULL font before
// setting (Martin Richter) C Maunder     - 15 Oct 2000 - GetTextExtent fixed
// (Martin Richter) C Maunder     -  1 Jan 2001 - Added ValidateEdit
//
...
//        the cells will behave in a certain way, and the cells trust the grid
//        will supply accurate information.
//
/////////////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "legacy/ui/mfc_ex/gridctrl/grid_cell_base.h"
...
CGridCellBase::CGridCellBase() { Reset(); }
...
void CGridCellBase::Reset() { m_nState = 0; }
...
CGridCellBase* CGridCellBase::GetDefaultCell() const {
  if (GetGrid()) return GetGrid()->GetDefaultCell(IsFixedRow(), IsFixedCol());
  return NULL;
}
...
// of bounds.
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,
  ...
  TextClr =
      (GetTextClr() == CLR_DEFAULT) ? pDefaultCell->GetTextClr() : GetTextClr();
  ...
  if (pGrid->GetSortColumn() == nCol && nRow == 0) {
    ...
    if (size.cx + rect.left < rect.right + (int)(2 * GetMargin())) {
      int nTriangleBase =
          rect.bottom - nOffset - size.cy;  // Triangle bottom right
      // int nTriangleBase = (rect.top + rect.bottom - size.cy)/2; // Triangle
      // middle right int nTriangleBase = rect.top + nOffset;                 //
      // Triangle top right

      // int nTriangleLeft = rect.right - size.cx;                 // Triangle
      // RHS int nTriangleLeft = (rect.right + rect.left - size.cx)/2; //
      // Triangle middle int nTriangleLeft = rect.left; // Triangle LHS

      int nTriangleLeft;
      if (bVertical)
        nTriangleLeft =
            (rect.right + rect.left - size.cx) / 2;  // Triangle middle
      else
      ...
      if (pGrid->GetSortAscending()) {
        ...
        pDC->LineTo(nTriangleLeft + (size.cx / 2) + 1,
                    nTriangleBase + size.cy + 1);
...
// Not yet implemented
void CGridCellBase::OnMouseEnter() { TRACE0("Mouse entered cell\n"); }
...
// Not Yet Implemented
void CGridCellBase::OnMouseLeave() { TRACE0("Mouse left cell\n"); }
...
void CGridCellBase::OnEndEdit() { ASSERT(FALSE); }
...
BOOL CGridCellBase::GetTextRect(
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/) {
  ...
  if ((nFormat & DT_WORDBREAK) && !(nFormat & DT_SINGLELINE)) {
    ...
    while (TRUE) {
      ...
      if (nTempWidth > nMaxWidth) nMaxWidth = nTempWidth;
  ...
  if (bReleaseDC) pGrid->ReleaseDC(pDC);
...
// printed correctly.
BOOL CGridCellBase::PrintCell(CDC* pDC, int /*nRow*/, int /*nCol*/,
                              CRect rect) {
#if defined(_WIN32_WCE_NO_PRINTING) || defined(GRIDCONTROL_NO_PRINTING)
  return FALSE;
#else
  ...
  {
      Font.CreateFont(plfFont->lfHeight, 0, 0, 0, FW_BOLD, plfFont->lfItalic, 0,
0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, #ifndef _WIN32_WCE
          PROOF_QUALITY,
#else
          DEFAULT_QUALITY,
#endif
