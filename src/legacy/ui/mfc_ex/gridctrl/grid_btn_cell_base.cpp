...
#include "legacy/ui/mfc_ex/gridctrl/in_place_edit.h"
...
#define ALL_BUT_BTN_CHK                                     \
  (DFCS_BUTTON3STATE | DFCS_BUTTONPUSH | DFCS_BUTTONRADIO | \
   DFCS_BUTTONRADIOIMAGE | DFCS_BUTTONRADIOMASK)
...
BOOL CGridBtnCellBase::SetupBtns(
    ...
    CGridBtnCellBase::CTL_ALIGN
        aAlign,            // horizontal alignment of control image
...
{
  if (aiWhich < 0 || aiWhich >= GetDrawCtlNbrMax()) {
    ASSERT(FALSE);
    return FALSE;
  }
  if (aAlign < 0 || aAlign >= CTL_ALIGN_FENCE) {
    ASSERT(FALSE);
    return FALSE;
  }

  if (aAlign == CTL_ALIGN_CENTER && aiWhich > 0) {
  ...
  if (aiWhich >= GetDrawCtlNbr()) {
    ...
    ASSERT(aiWhich ==
           GetDrawCtlNbr());  // if ASSERT, then you are setting
...
BOOL CGridBtnCellBase::DrawBtnCell(CDC* pDC, int /* nRow */, int /* nCol */,
                                   CRect* prect, BOOL /* bEraseBkgnd */) {
  CGridCtrl* pGrid = GetGrid();

  if (!pGrid || !pDC) return FALSE;
  ...
  if (iCtlNbr > 0) {
    if (GetState() & GVIS_SELECTED || GetState() & GVIS_DROPHILITED) {
                         ...
                         *prect))              // cell rectangle to work with
    {
      for (int i1 = 0; i1 < iCtlNbr; i1++) {
        ...
        pDC->DrawFrameControl(RectAry[i1], uiType, uiState);
...
*****************************************************************************/
BOOL CGridBtnCellBase::Edit(int nRow, int nCol, CRect rect, CPoint point,
                            UINT nID, UINT nChar) {
  ASSERT(m_pBtnDataBase != NULL);

  if (ProcessCtlClick(
          WM_LBUTTONUP,  // Command that invoked.  e.g. WM_LBUTTONDOWN
  ...
  if (iCtlNbr > 0) {
    ...
    if (GetDrawCtlAlign(0) == CTL_ALIGN_LEFT || iCtlNbr == 1) {
      if (nChar == ' ') {
        // user hit space bar -- just like clicking a button
        ClickedCellCtl(
            WM_LBUTTONDOWN,  // Command that invoked.  e.g. WM_LBUTTONDOWN
  ...
  if (HasCellText()) {
    ...
    CGridCtrl* pGrid = GetGrid();
    CWnd* pWnd = new CInPlaceEdit(pGrid, rect, dwStyle, nID, nRow, nCol,
                                  GetText(), nChar);
...
void CGridBtnCellBase::EndEdit() {
  ...
  if (pWnd) ((CInPlaceEdit*)pWnd)->EndEdit();
}
...
void CGridBtnCellBase::OnDblClick(CPoint PointCellRelative) {
  ProcessCtlClick(
      WM_LBUTTONDBLCLK,    // Command that invoked.  e.g. WM_LBUTTONDOWN
      PointCellRelative);  // point to check for hit
}

BOOL CGridBtnCellBase::GetTextRect(
    LPRECT pRect)  // i/o:  i=dims of cell rect; o=dims of text rect
{
  CGridCtrl* pGrid = GetGrid();
  ASSERT(pGrid);
    ...
    for (int i1 = 0; i1 < iCtlNbr; i1++) {
      ...
      if (iWidth <= 0) {
        ...
        if (pszBtnText == NULL) {
          ...
          pDC->DrawText(pszBtnText, strlen(pszBtnText), &rectText,
                        GetFormat() | DT_CALCRECT);
...
*****************************************************************************/
BOOL CGridBtnCellBase::ClickedCellCtl(
    UINT uMsg,    // Command that invoked.  e.g. WM_LBUTTONDOWN
    int aiWhich)  // zero-based index of image to draw
  ...
  switch (uMsg) {
    ...
    case WM_LBUTTONDBLCLK:
      ...
      if (iType == DFC_BUTTON) {
        ...
        if (uiState & DFCS_BUTTONRADIO || bIsMbrRadioGrp) {
          ...
          for (int i1 = 0; i1 < iCtlNbr; i1++) {
            if (i1 != aiWhich) {
              ...
              if (uiStateRadio & DFCS_BUTTONRADIO || bIsMbrRadioGrp) {
...
*****************************************************************************/
BOOL CGridBtnCellBase::ProcessCtlClick(
    UINT uMsg,              // Command that invoked.  e.g. WM_LBUTTONDOWN
...
// returns:  T=hit a control / F=no control hit
{
  ...
  // returns:  Index of control that this point is within bounds of or -1 if no
  // control matches
...
int CGridBtnCellBase::RelPointInCtl(
    const CPoint& arPoint)  // Relative point coords
// returns:  Index of control that this point is within bounds of or -1 if no
// control matches
{
  CGridCtrl* pGrid = GetGrid();
  ...
  CRect RectCell;
  if (pGrid->GetCellRect(m_iRow, m_iCol, &RectCell)) {
    ASSERT(MAX_NBR_CTLS_INCELL > GetDrawCtlNbrMax());  // whoa!
    CRect RectAry[MAX_NBR_CTLS_INCELL];
                         ...
                         RectCell))            // cell rectangle to work with
    {
      ...
      for (int i1 = 0; i1 < iCtlNbr; i1++) {
        if (pointAbs.x >= RectAry[i1].left && pointAbs.x <= RectAry[i1].right &&
            pointAbs.y >= RectAry[i1].top && pointAbs.y <= RectAry[i1].bottom) {
          return i1;  // found it
        }
      }
    }
...
*****************************************************************************/
BOOL CGridBtnCellBase::CalcDrawCtlRects(
    CRect* apRect,            // returns:  CRects with coordinates
                              //  last entry has optional leftover rect
                              //  available for text, etc.
...
// returns:  success / fail
{
  ...
  const int iCtlNbr = GetDrawCtlNbr();
  if (iCtlNbr <= 0) return FALSE;

  int i1, i2;
  int iSpinBoxDownIdx =
      -1;  // can have zero or 1 spin box -- no more
  ...
  for (i1 = 0; i1 < iCtlNbr; i1++) {
    ...
    if (iSpinBoxDownIdx < 0) {
      ...
      if (GetDrawCtlType(i1) == DFC_SCROLL &&
          !(uiState &
            (DFCS_SCROLLCOMBOBOX | DFCS_SCROLLDOWN | DFCS_SCROLLLEFT |
             DFCS_SCROLLRIGHT |
  ...
  for (i1 = 0; i1 < iCtlNbr; i1++) {
    ...
    if (iWidth > 0)
      ...
      if (!bIsRectangle && arRectCell.Height() < iSizeToFitWidth) {
  ...
  apRect->top =
      iSavedTop + GetMargin();  // put some margin so doesn't overwrite
  iSavedTop += apRect->Width();
  apRect->bottom = iSavedTop;
  ...
  for (i1 = 0; i1 <= iCtlNbr; i1++)  // note that I'll get leftover rect, too
  {
    ...
    if (apRect->left > arRectCell.right) apRect->left = arRectCell.right;

    apRect->right = apRect->bottom;
    if (apRect->right > arRectCell.right) apRect->right = arRectCell.right;
    if (apRect->right < arRectCell.left) apRect->right = arRectCell.left;
...
LPCTSTR CGridBtnCellBase::GetTipText() {
  if (HasCellText()) return GetText();

  // no string text... maybe there's btns with text
  const int iCtlNbr = GetDrawCtlNbr();
  if (iCtlNbr <= 0) return NULL;
...
*****************************************************************************/
BOOL CGridBtnCellBase::GetTipTextRect(
    LPRECT pRect)  // i/o:  i=dims of cell rect; o=dims of text rect
{
  if (HasCellText())
...
// returns:  F=auto-size buttons, only
{
  ...
  CRect RectCell;
  if (!pGrid->GetCellRect(m_iRow, m_iCol, &RectCell)) return FALSE;

  // rather than see if there is text assigned, check if any
  //  space allocated for text
  CRect RectText(RectCell);
  if (!GetTextRect(
          &RectText))  // i/o:  i=dims of cell rect; o=dims of text rect
    return FALSE;

  if (RectText.Width() > 0) return TRUE;
...
// returns:  index of button or -1 if no hot key matches
{
  ...
  for (int i1 = 0; i1 < iCtlNbr; i1++) {
    ...
    while (*pszWork != NULL) {
      if (*pszWork == '&') {
        pszWork++;
        if (*pszWork == NULL) break;

        if (tolower(*pszWork) == tolower(acHotKey)) return i1;
      }
