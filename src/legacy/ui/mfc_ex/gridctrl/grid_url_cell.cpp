// grid_url_cell.cpp: implementation of the CGridURLCell class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "legacy/ui/mfc_ex/gridctrl/grid_url_cell.h"
...
BOOL CGridURLCell::Draw(CDC* pDC, int nRow, int nCol, CRect rect,
                        BOOL bEraseBkgnd) {
  // If url is present then change text color
  if (HasUrl(GetText())) SetTextClr(m_clrUrl);
...
BOOL CGridURLCell::Edit(int nRow, int nCol, CRect rect, CPoint point, UINT nID,
                        UINT nChar) {
  return FALSE;
}
...
BOOL CGridURLCell::HasUrl(CString str) {
  ...
  for (int i = 0; i < nNumPrefixes; i++)
    ...
    if (str.Find(g_szURIprefixes[i].szURLPrefix) >= 0) return TRUE;
...
BOOL CGridURLCell::OverURL(CPoint& pt, CString& strURL) {
  ...
  if ((m_nFormat & DT_RIGHT) && pt.x >= (m_Rect.right - size.cx)) {
    bOverURL = TRUE;
  } else if ((m_nFormat & DT_CENTER) && ((center.x - (size.cx / 2)) <= pt.x) &&
  ...
  if (m_nFormat & DT_CENTER) {
    int wide = m_Rect.Width() - size.cx;
    pt.x -= (wide / 2);
    if (pt.x <= 0 || pt.x > (size.cx + (wide / 2))) return FALSE;
  }
  ...
  if (endSpace != -1) strURL.Delete(endSpace, strURL.GetLength() - endSpace);

  int beginSpace = strURL.ReverseFind(_T(' '));
  if (beginSpace != -1) strURL.Delete(0, ++beginSpace);
#endif
