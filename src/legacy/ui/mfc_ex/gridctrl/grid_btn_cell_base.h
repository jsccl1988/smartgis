...
*****************************************************************************/
#if !defined( \
    AFX_GridBtnCellBase_H__97838AF2_EBD9_11D3_B75D_00C04F6A7AE6__INCLUDED_)
...
class AFX_EXT_CLASS CGridBtnCellBase : public CGridCellBase {
 ...
 public:
  ...
  BOOL SetupBtns(
      ...
      const char* apszText);  // Text to insert centered in button; if NULL no
                              // text returns:  success / failure
 ...
 public:
  ...
  virtual void SetDrawCtlBtnText(int /* aiWhich */,
                                 const char* /* apszText */) {
    ASSERT(FALSE);
  }
 ...
 public:
  ...
  virtual LPCTSTR GetText() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetText(m_iRow, m_iCol);
    return NULL;
  }
  ...
  virtual int GetImage() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetImage(m_iRow, m_iCol);
    return -1;
  }

  virtual LPARAM GetData() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetData(m_iRow, m_iCol);
    return 0;
  }

  virtual DWORD GetFormat() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetFormat(m_iRow, m_iCol);
    return 0;
  }

  virtual COLORREF GetTextClr() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetTextClr(m_iRow, m_iCol);
    return CLR_DEFAULT;
  }

  virtual COLORREF GetBackClr() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetBackClr(m_iRow, m_iCol);
    return CLR_DEFAULT;
  }

  virtual LOGFONT* GetFont() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetFont(m_iRow, m_iCol);
    return NULL;
  }

  virtual CFont* GetFontObject() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetFontObject(m_iRow, m_iCol);
    return NULL;
  }

  virtual UINT GetMargin() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetMargin(m_iRow, m_iCol);
    return 0;
  }

  virtual CGridCtrl* GetGrid() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetGrid();
    return NULL;
  }

  virtual CWnd* GetEditWnd() const {
    if (m_pBtnDataBase) return m_pBtnDataBase->GetEditWnd();
    return NULL;
  }
 ...
 public:
  ...
  BOOL DrawBtnCell(CDC* pDC, int nRow, int nCol, CRect* prect,
                   BOOL bEraseBkgnd);
  virtual BOOL GetTextRect(
 ...
 protected:
  ...
  CBtnDataBase* m_pBtnDataBase;  // contains data common to this instance of the
                                 // grid as well
                                 //  as a reference to the grid, too

  static CString
      m_strTipText;  // may have special title tip - note only 1 tip at a time
};
