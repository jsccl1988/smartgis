// grid_ctrl.cpp : implementation file
//
...
//                          there is a single line of Joe's code left. Many,
//                          many, MANY people sent in bug reports and fixes.
//                          Thank you all.
//
//          2.0         1 Feb 2000
//                          Rewritten to make the grid more object oriented, in
//                          that the CGridCell class now takes care of
//                          cell-specific tasks. This makes the code more
//                          robust, but more importantly it allows the simple
//                          insertion of other types of cells.
//
//          2.10       11 Mar 2000 - Ken Bertelson and Chris Maunder
//                          - Additions for virtual CGridCell support of
//                          embedded tree
//                            & cell buttons implementation
//                          - Optional WYSIWYG printing
//                          - Awareness of hidden (0 width/height) rows and
//                          columns for
//                            key movements,  cut, copy, paste, and autosizing
//                          - CGridCell can make title tips display any text
//                          rather than
//                            cell text only
//                          - Minor vis bug fixes
//                          - CGridCtrl now works with CGridCellBase instead of
//                          CGridCell
//                            This is a taste of things to come.
//
//          2.20       30 Jul 2000 - Chris Maunder
//                          - Font storage optimised (suggested by Martin
//                          Richter)
//                          - AutoSizeColumn works on either column header, data
//                          or both
//                          - EnsureVisible. The saga continues... (Ken)
//                          - Rewrote exception handling
//                          - Added TrackFocusCell and FrameFocusCell
//                          properties, as well as
//                            ExpandLastColumn (suggested by Bruce E.
//                            Stemplewski).
//                          - InsertColumn now allows you to insert columns at
//                          the end of the
//                            column range (David Weibel)
//                          - Shift-cell-selection more intuitive
//                          - API change: Set/GetGridColor now
//                          Set/GetGridLineColor
//                          - API change: Set/GetBkColor now Set/GetGridBkColor
//                          - API change: Set/GetTextColor, Set/GetTextBkColor
//                          depricated
//                          - API change: Set/GetFixedTextColor,
//                          Set/GetFixedBkColor depricated
//                          - Stupid DDX_GridControl workaround removed.
//                          - Added "virtual mode" via Set/GetVirtualMode
//                          - Added SetCallbackFunc to allow callback functions
//                          in virtual mode
...
//                          - GetMouseScrollLines now uses win98/W2K friendly
//                          code
//                          - WS_EX_CLIENTEDGE style now implicit
//
//                          [ Only the latest version and major version changes
//                          will be shown ]
//
...
//         increases
//            with time.
//         3) Scrolling is still a little dodgy (too much grey area). I know
//         there
//            is a simple fix but it's been a low priority
//         4) Get some sleep
//
// ISSUES: 2) WindowFromPoint seems to do weird things in W2K. Causing problems
// for
...
#include "legacy/ui/mfc_ex/gridctrl/mem_dc.h"
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols) {
  ...
  if (!pState->m_bNeedTerm && !AfxOleInit())
    AfxMessageBox(
        _T("OLE initialization failed. Make sure that the OLE libraries are ")
        _T("the correct version"));
#endif
  ...
  m_nHScrollMax = 0;
  m_nRowsPerWheelNotch =
      GetMouseScrollLines();  // Get the number of lines
  ...
  m_nAutoSizeColumnStyle =
      GVS_BOTH;  // Autosize grid using header and data info
...
//        m_TitleTip.DestroyWindow();
  ...
  if (pSource) COleDataSource::FlushClipboard();
#endif
}
...
BOOL CGridCtrl::RegisterWindowClass() {
  ...
  HINSTANCE g_hInstDLL = hInstOld;
  AfxSetResourceHandle(g_hInstDLL);
  HINSTANCE hInst = g_hInstDLL;
...
//    m_TitleTip.SetParentWnd(this);
  ...
  if (::IsWindow(m_hWnd)) ModifyStyleEx(0, WS_EX_CLIENTEDGE);
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID,
  ...
  // was called with non-zero initialising parameters. If this window was
  // created using a dialog template then the number of rows and columns will be
  // 0 (which means that the code below will not be needed - which is lucky
  // 'cause it ain't gonna get called in a dialog-template-type-situation.
  ...
  int i;
  for (i = 0; i < m_nRows; i++) m_arRowHeights[i] = m_cellDefault.GetHeight();
  for (i = 0; i < m_nCols; i++) m_arColWidths[i] = m_cellDefault.GetWidth();
...
void CGridCtrl::SetupDefaultCells() {
  ...
  m_cellFixedRowColDef.SetGrid(
      this);  // Cell for area overlapped by fixed columns/rows
...
/*BOOL CGridCtrl::SetRowFocusAndSelection(int nRow,int nCol=0)
{
  CGridCtrl(nRow, nCol);
  return true;
}*/
...
// erase the background normally through OnEraseBkgnd, and didn't fill the
// memDC's selected bitmap with colour, then all sorts of vis problems would
// occur
void CGridCtrl::EraseBkgnd(CDC* pDC) {
  ...
  if (ClipRect.left < nFixedColumnWidth && ClipRect.top < nFixedRowHeight)
    pDC->FillRect(
        CRect(ClipRect.left, ClipRect.top, nFixedColumnWidth, nFixedRowHeight),
        &FixedRowColBack);

  // Draw Fixed columns background
  if (ClipRect.left < nFixedColumnWidth && ClipRect.top < VisRect.bottom)
    pDC->FillRect(
        CRect(ClipRect.left, ClipRect.top, nFixedColumnWidth, VisRect.bottom),
        &FixedColBack);

  // Draw Fixed rows background
  if (ClipRect.top < nFixedRowHeight && ClipRect.right > nFixedColumnWidth &&
      ClipRect.left < VisRect.right)
    pDC->FillRect(CRect(nFixedColumnWidth - 1, ClipRect.top, VisRect.right,
                        nFixedRowHeight),
                  &FixedRowBack);
  ...
  if (rect.IntersectRect(VisRect, ClipRect)) {
    CRect CellRect(max(nFixedColumnWidth, rect.left),
                   max(nFixedRowHeight, rect.top), rect.right, rect.bottom);
  ...
  if (VisRect.right < ClipRect.right)
    pDC->FillRect(
        CRect(VisRect.right, ClipRect.top, ClipRect.right, ClipRect.bottom),
        &Back);

  // Draw bottom of window below grid
  if (VisRect.bottom < ClipRect.bottom && ClipRect.left < VisRect.right)
    pDC->FillRect(
        CRect(ClipRect.left, VisRect.bottom, VisRect.right, ClipRect.bottom),
        &Back);
}
...
UINT CGridCtrl::OnGetDlgCode() {
  ...
  if (m_bHandleTabKey && !IsCTRLpressed()) nCode |= DLGC_WANTTAB;
...
void CGridCtrl::OnCaptureChanged(CWnd* pWnd) {
  ...
  if (m_MouseMode == MOUSE_DRAGGING) m_MouseMode = MOUSE_NOTHING;
#endif
}
#endif
...
#ifdef _WIN32_WCE
  if (m_MouseMode == MOUSE_NOTHING) return;
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
  if (IsSHIFTpressed() && (nChar == VK_INSERT)) OnEditPaste();
#endif
  ...
  if (nChar == VK_DELETE) {
    ...
    if (!bFoundVisible) next.row = iOrig;
  } else if (nChar == VK_UP) {
    // don't let user go to a hidden row
    bFoundVisible = FALSE;
    iOrig = next.row;
    ...
    if (nChar == VK_TAB) {
      // If we're at the end of a row, go down a row till we find a non-hidden
      // row
    ...
    if (!bFoundVisible) next.col = iOrig;
    ...
    if (nChar == VK_TAB) {
      if (next.col == (GetFixedColumnCount() - 1) &&
    ...
    if (increment) {
      next.row += increment;
      if (next.row > (GetRowCount() - 1)) next.row = GetRowCount() - 1;
    } else
    ...
    while (next.col < GetColumnCount() - 1) {
      if (GetColumnWidth(next.col) > 0) break;
      next.col++;
    }
    while (next.row < GetRowCount() - 1) {
      if (GetRowHeight(next.row) > 0) break;
      next.row++;
    }
    ...
    OnEditCell(m_idCurrentCell.row, m_idCurrentCell.col, CPoint(-1, -1),
               VK_LBUTTON);
  } else {
    CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
    return;
  ...
  if (next != m_idCurrentCell) {
    ...
    if (m_MouseMode == MOUSE_NOTHING) {
      ...
      if (!IsSHIFTpressed() || nChar == VK_TAB) m_SelectionStartCell = next;

      // Notify parent that selection is changing - Arthur Westerman/Scot
      // Brennecke
    ...
    if (bHorzScrollAction) SendMessage(WM_HSCROLL, SB_ENDSCROLL, 0);
    if (bVertScrollAction) SendMessage(WM_VSCROLL, SB_ENDSCROLL, 0);
  }
}
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/,
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/,
                          CScrollBar* /*pScrollBar*/) {
  EndEditing();
...
void CGridCtrl::OnDraw(CDC* pDC) {
  ...
  if (GetVirtualMode()) SendCacheHintToParent(VisCellRange);
...
void CGridCtrl::SetRedraw(BOOL bAllowDraw,
  ...
  //             GetRuntimeClass()->m_lpszClassName, bAllowDraw? _T("TRUE") :
  //             _T("FALSE"));
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */) {
  ...
  if (pDC) {
    ...
    if (nRow < m_nFixedRows || nCol < m_nFixedCols) {
      ...
      if (pCell) bResult = pCell->Draw(pDC, nRow, nCol, rect, TRUE);
    } else {
      CGridCellBase* pCell = GetCell(nRow, nCol);
      if (pCell) bResult = pCell->Draw(pDC, nRow, nCol, rect, TRUE);

      // Since we have erased the background, we will need to redraw the
      // gridlines
      CPen pen;
      pen.CreatePen(PS_SOLID, 0, m_crGridLineColour);
...
void CGridCtrl::SetSelectedRange(const CCellRange& Range,
  ...
  SetSelectedRange(Range.GetMinRow(), Range.GetMinCol(), Range.GetMaxRow(),
                   Range.GetMaxCol(), bForceRepaint, bSelectCells);
}

void CGridCtrl::SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow,
  ...
  if (IsWindow(GetSafeHwnd())) VisCellRange = GetVisibleNonFixedCellRange();
  ...
  // If we are selecting cells, then first clear out the list of currently
  // selected cells, then
  if (bSelectCells) {
    POSITION pos;
    ...
    for (pos = m_SelectedCellMap.GetStartPosition(); pos != NULL;) {
      ...
      if (IsValid(cell)) {
        ...
        if (nMinRow <= cell.row && cell.row <= nMaxRow && nMinCol <= cell.col &&
            cell.col <= nMaxCol)
          continue;
    ...
    // if we are selecting cells, and there are previous selected cells to be
    // retained (eg Ctrl is being held down) then copy them to the newly created
    // list, and mark all these cells as selected Note that if we are list mode,
    // single row selection, then we won't be adding the previous cells. Only
    // the current row of cells will be added (see below)
    if (!GetSingleRowSelection() && nMinRow >= 0 && nMinCol >= 0 &&
        nMaxRow >= 0 && nMaxCol >= 0) {
  ...
  // Now select/deselect all cells in the cell range specified. If selecting,
  // and the cell has already been marked as selected (above) then ignore it. If
  // we are deselecting and the cell isn't selected, then ignore
  if (nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0 &&
      nMaxRow < m_nRows && nMaxCol < m_nCols && nMinRow <= nMaxRow &&
      nMinCol <= nMaxCol) {
...
void CGridCtrl::SelectAllCells() {
  ...
  SetSelectedRange(m_nFixedRows, m_nFixedCols, GetRowCount() - 1,
                   GetColumnCount() - 1);
}

// selects columns
void CGridCtrl::SelectColumns(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/,
                              BOOL bSelectCells /*=TRUE*/) {
  if (!m_bEnableSelection) return;
  ...
  if (GetSingleColSelection())
    SetSelectedRange(GetFixedRowCount(), currentCell.col, GetRowCount() - 1,
                     currentCell.col, bForceRedraw, bSelectCells);
  else
    SetSelectedRange(
        GetFixedRowCount(), min(m_SelectionStartCell.col, currentCell.col),
        GetRowCount() - 1, max(m_SelectionStartCell.col, currentCell.col),
        bForceRedraw, bSelectCells);
}

// selects rows
void CGridCtrl::SelectRows(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/,
                           BOOL bSelectCells /*=TRUE*/) {
  if (!m_bEnableSelection) return;
  ...
  if (GetSingleRowSelection())
    SetSelectedRange(currentCell.row, GetFixedColumnCount(), currentCell.row,
                     GetColumnCount() - 1, bForceRedraw, bSelectCells);
  else
...
// selects cells
void CGridCtrl::SelectCells(CCellID currentCell, BOOL bForceRedraw /*=FALSE*/,
                            BOOL bSelectCells /*=TRUE*/) {
  if (!m_bEnableSelection) return;
  ...
  SetSelectedRange(
      min(m_SelectionStartCell.row, row), min(m_SelectionStartCell.col, col),
...
void CGridCtrl::OnSelecting(const CCellID& currentCell) {
  ...
  // SetFocusCell(max(currentCell.row, m_nFixedRows), max(currentCell.col,
  // m_nFixedCols));
}

void CGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol,
                                              LPCTSTR strText) {
  if (!IsCellEditable(nRow, nCol)) return;
...
COleDataSource* CGridCtrl::CopyTextFromGrid() {
  ...
  if (GetVirtualMode()) SendCacheHintToParent(Selection);

  // Write to shared file (REMEBER: CF_TEXT is ANSI, not UNICODE, so we need to
  // convert)
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject,
  ...
  do {
    ...
    CString strCellText =
        (nLineIndex >= 0) ? strLine.Left(nLineIndex) : strLine;

    // skip hidden rows
    int iRowVis = cell.row + nLine;
    while (iRowVis < GetRowCount()) {
      if (GetRowHeight(iRowVis) > 0) break;
      nLine++;
      iRowVis++;
    }
    ...
    while (!strLine.IsEmpty()) {
      ...
      while (iColVis < GetColumnCount()) {
        if (GetColumnWidth(iColVis) > 0) break;
        nColumn++;
        iColVis++;
      }
      ...
      if (IsValid(TargetCell)) {
        ...
        ValidateAndModifyCellContents(TargetCell.row, TargetCell.col,
                                      strCellText);
...
void CGridCtrl::OnBeginDrag() {
  ...
  if (pSource) {
    SendMessageToParent(GetSelectedCellRange().GetTopLeft().row,
                        GetSelectedCellRange().GetTopLeft().col, GVN_BEGINDRAG);
    ...
    if (dropEffect & DROPEFFECT_MOVE) CutSelectedText();
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState,
  ...
  // If not valid, set the previously drop-highlighted cell as no longer
  // drop-highlighted
  if (!IsValid(cell)) {
    OnDragLeave();
  ...
  if (!IsCellEditable(cell)) return DROPEFFECT_NONE;
  ...
  if (cell != m_LastDragOverCell) {
    ...
    if (IsValid(m_LastDragOverCell)) {
      UINT nState =
          GetItemState(m_LastDragOverCell.row, m_LastDragOverCell.col);
...
DROPEFFECT CGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
  ...
  if (!IsValid(m_LastDragOverCell)) return DROPEFFECT_NONE;

  if (!IsCellEditable(m_LastDragOverCell)) return DROPEFFECT_NONE;
...
BOOL CGridCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT /*dropEffect*/,
  ...
  if (!m_bAllowDragAndDrop || !IsCellEditable(m_LastDragOverCell)) return FALSE;
...
void CGridCtrl::OnEditPaste() {
  ...
  // Get the top-left selected cell, or the Focus cell, or the topleft
  // (non-fixed) cell
  CCellID cell;
  ...
  COleDataObject obj;
  if (obj.AttachClipboard()) PasteTextToGrid(cell, &obj);
}
#endif

void CGridCtrl::OnEditSelectAll() {
  SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col,
                      GVN_SELCHANGING);
  SelectAllCells();
...
void CGridCtrl::OnUpdateEditCut(CCmdUI* pCmdUI) {
  pCmdUI->Enable(IsEditable() &&
                 GetSelectedCount() >
                     0);  // - Thomas Haase
                          // CCellRange Selection = GetSelectedCellRange();
                          // pCmdUI->Enable(IsEditable() && Selection.Count() &&
                          // IsValid(Selection));
}
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point) {
  if (point.x >= GetFixedColumnWidth()) return FALSE;

  CCellID idCurrentCell = GetCellFromPt(point);
  CPoint start;
  if (!GetCellOrigin(idCurrentCell, &start)) return FALSE;
...
BOOL CGridCtrl::MouseOverColumnResizeArea(CPoint& point) {
  if (point.y >= GetFixedRowHeight()) return FALSE;
...
CCellID CGridCtrl::GetCellFromPt(CPoint point,
  ...
  CCellID idTopLeft = GetTopleftNonFixedCell();
  if (!bAllowFixedCellCheck && !IsValid(idTopLeft)) return cellID;

  // calculate column index
  int fixedColWidth = GetFixedColumnWidth();

  if (point.x < 0 ||
      (!bAllowFixedCellCheck && point.x < fixedColWidth))  // not in window
    cellID.col = -1;
  ...
  if (point.y < 0 ||
      (!bAllowFixedCellCheck && point.y < fixedRowHeight))  // not in window
    cellID.row = -1;
...
CCellID CGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/) {
  ...
  if (m_idTopLeftCell.IsValid() && !bForceRecalculation) return m_idTopLeftCell;
...
CCellRange CGridCtrl::GetVisibleNonFixedCellRange(
...
// If resizing or cell counts/sizes change, call this - it'll fix up the scroll
// bars
void CGridCtrl::ResetScrollBars() {
  ...
  CRect VisibleRect(GetFixedColumnWidth(), GetFixedRowHeight(), rect.right,
                    rect.bottom);
  ...
  ASSERT(m_nVScrollMax < INT_MAX &&
         m_nHScrollMax < INT_MAX);  // This should be fine
...
BOOL CGridCtrl::GetCellOrigin(int nRow, int nCol, LPPOINT p) {
  ...
  if (nCol < m_nFixedCols)  // is a fixed column
    for (i = 0; i < nCol; i++) p->x += GetColumnWidth(i);
  else {  // is a scrollable data column
    for (i = 0; i < m_nFixedCols; i++) p->x += GetColumnWidth(i);
    for (i = idTopLeft.col; i < nCol; i++) p->x += GetColumnWidth(i);
  }

  p->y = 0;
  if (nRow < m_nFixedRows)  // is a fixed row
    for (i = 0; i < nRow; i++) p->y += GetRowHeight(i);
  else {  // is a scrollable data row
    for (i = 0; i < m_nFixedRows; i++) p->y += GetRowHeight(i);
    for (i = idTopLeft.row; i < nRow; i++) p->y += GetRowHeight(i);
  }
...
BOOL CGridCtrl::GetCellRect(int nRow, int nCol, LPRECT pRect) {
  ...
  //       nRow,nCol, CellOrigin.x, CellOrigin.y, GetColumnWidth(nCol),
  //       GetRowHeight(nRow), pRect->left, pRect->top, pRect->right,
  //       pRect->bottom);
...
LRESULT CGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */) {
  ...
  LOGFONT lf;
  if (!GetObject((HFONT)hFont, sizeof(LOGFONT), &lf)) return result;
    ...
    switch (m_MouseMode) {
      ...
      default:
        if (!GetVirtualMode()) {
          ...
          if (IsValid(cell)) {
            CGridCellBase* pCell = GetCell(cell.row, cell.col);
            if (pCell) return pCell->OnSetCursor();
          }
        }
...
BOOL CGridCtrl::SetFixedRowCount(int nFixedRows) {
  if (m_nFixedRows == nFixedRows) return TRUE;
  ...
  if (m_idCurrentCell.row < nFixedRows) SetFocusCell(-1, -1);
    ...
    if (nFixedRows > m_nFixedRows) {
      ...
      for (int i = nFixedRows; i < m_nFixedRows; i++) {
        ...
        for (j = GetFixedColumnCount(); j < GetColumnCount(); j++) {
          SetItemState(i, j,
                       GetItemState(i, j) & ~(GVIS_FIXED | GVIS_FIXEDROW));
          SetItemBkColour(i, j, CLR_DEFAULT);
          SetItemFgColour(i, j, CLR_DEFAULT);
        }
      }
...
BOOL CGridCtrl::SetFixedColumnCount(int nFixedCols) {
  if (m_nFixedCols == nFixedCols) return TRUE;
  ...
  if (m_idCurrentCell.col < nFixedCols) SetFocusCell(-1, -1);
    ...
    if (nFixedCols > m_nFixedCols) {
      ...
      {  // Scope limit i,j
        for (int i = GetFixedRowCount(); i < GetRowCount(); i++)
          for (int j = nFixedCols; j < m_nFixedCols; j++) {
            SetItemState(i, j,
                         GetItemState(i, j) & ~(GVIS_FIXED | GVIS_FIXEDCOL));
            SetItemBkColour(i, j, CLR_DEFAULT);
            SetItemFgColour(i, j, CLR_DEFAULT);
          }
      }
...
BOOL CGridCtrl::SetRowCount(int nRows) {
  ...
  if (nRows < m_nFixedRows) m_nFixedRows = nRows;

  if (m_idCurrentCell.row >= nRows) SetFocusCell(-1, -1);
  ...
  if (addedRows < 0) {
    if (!GetVirtualMode()) {
      for (int row = nRows; row < m_nRows; row++) {
        // Delete cells
        for (int col = 0; col < m_nCols; col++) DestroyCell(row, col);
  ...
  TRY {
    ...
    if (GetVirtualMode()) {
      ...
      // If we have just added rows, we need to construct new elements for each
      // cell and set the default row height
      if (addedRows > 0) {
...
BOOL CGridCtrl::SetColumnCount(int nCols) {
  ...
  if (nCols == GetColumnCount()) return bResult;

  // Force recalculation
  m_idTopLeftCell.col = -1;

  if (nCols < m_nFixedCols) m_nFixedCols = nCols;

  if (m_idCurrentCell.col >= nCols) SetFocusCell(-1, -1);
  ...
  TRY {
    ...
    // If we have just added columns, we need to construct new elements for each
    // cell and set the default column width
    if (addedCols > 0) {
...
// Insert a column at a given position, or add to end of columns (if nColumn =
// -1)
int CGridCtrl::InsertColumn(
    LPCTSTR strHeading, UINT nFormat /* = DT_CENTER|DT_VCENTER|DT_SINGLELINE */,
    int nColumn /* = -1 */) {
  ...
  // "-1" column, meaning it gets flaged as being the last column, and not a
  // regular "insert" routine.
  if (nColumn == GetColumnCount()) nColumn = -1;
  ...
  for (int row = 0; row < m_nRows; row++) SetItemFormat(row, nColumn, nFormat);
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */) {
  ...
  TRY {
    ...
    if (nRow < 0) {
      ...
      if (!GetVirtualMode()) m_RowData.Add(new GRID_ROW);
    } else {
      m_arRowHeights.InsertAt(nRow, (int)0, 1);
      if (!GetVirtualMode()) m_RowData.InsertAt(nRow, new GRID_ROW);
    }

    if (!GetVirtualMode()) m_RowData[nRow]->SetSize(m_nCols);
  }
  CATCH(CMemoryException, e) {
    e->ReportError();
    return FALSE;
...
BOOL CGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass) {
  if (GetVirtualMode()) return FALSE;
  ...
  if (pCurrCell) *pNewCell = *pCurrCell;
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol) {
  ...
  if (!m_pRtcDefault ||
      !m_pRtcDefault->IsDerivedFrom(RUNTIME_CLASS(CGridCellBase))) {
    ASSERT(FALSE);
    return NULL;
  }
...
BOOL CGridCtrl::DeleteRow(int nRow) {
  ...
  if (!GetVirtualMode()) {
    ...
    for (int col = 0; col < GetColumnCount(); col++) DestroyCell(nRow, col);
...
BOOL CGridCtrl::DeleteNonFixedRows() {
  ...
  for (int nRow = nCount; nRow >= nFixed; nRow--) DeleteRow(nRow);
...
CCellID CGridCtrl::GetNextItem(CCellID& cell, int nFlags) const {
  if ((nFlags & GVNI_ALL) ==
      GVNI_ALL) {  // GVNI_ALL Search whole Grid beginning from cell
    ...
    for (; row < GetRowCount(); row++) {
      ...
      col = GetFixedColumnCount();
             ...
             (nFlags & GVNI_TORIGHT) ==
                 GVNI_TORIGHT) {  // GVNI_AREA Search Grid beginning from cell
                                  // to Lower-Right of Grid
...
// Sorts on a given column using the supplied compare function (see
// CListCtrl::SortItems)
...
int CALLBACK CGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2,
                                           LPARAM lParamSort) {
...
int CALLBACK CGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2,
                                              LPARAM lParamSort) {
  UNUSED_ALWAYS(lParamSort);
...
BOOL CGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending,
  ...
  while (lo <= hi) {
    ...
    if (bAscending)
      while (lo < high &&
             pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)pMidCell, data) < 0)
        ++lo;
    else
      while (lo < high &&
             pfnCompare((LPARAM)GetCell(lo, nCol), pMidCell, data) > 0)
        ++lo;
    ...
    if (bAscending)
      while (hi > low &&
             pfnCompare((LPARAM)GetCell(hi, nCol), pMidCell, data) > 0)
        --hi;
    else
      while (hi > low &&
             pfnCompare((LPARAM)GetCell(hi, nCol), pMidCell, data) < 0)
    ...
    if (lo <= hi) {
      ...
      if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol),
                     data) != 0) {
        for (int col = 0; col < GetColumnCount(); col++) {
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem) {
  if (!pItem || GetVirtualMode()) return FALSE;
...
BOOL CGridCtrl::SetItemBkColour(int nRow, int nCol,
...
BOOL CGridCtrl::SetItemFgColour(int nRow, int nCol,
                                COLORREF cr /* = CLR_DEFAULT */) {
  if (GetVirtualMode()) return FALSE;
...
const LOGFONT* CGridCtrl::GetItemFont(int nRow, int nCol) {
  ...
  ASSERT(pCell);
  if (!pCell)
    return GetDefaultCell(nRow < GetFixedRowCount(),
...
long CGridCtrl::GetVirtualWidth() const {
  ...
  for (int i = 0; i < iColCount; i++) lVirtualWidth += m_arColWidths[i];
...
long CGridCtrl::GetVirtualHeight() const {
  ...
  for (int i = 0; i < iRowCount; i++) lVirtualHeight += m_arRowHeights[i];
...
int CGridCtrl::GetFixedRowHeight() const {
  int nHeight = 0;
  for (int i = 0; i < m_nFixedRows; i++) nHeight += GetRowHeight(i);
...
int CGridCtrl::GetFixedColumnWidth() const {
  int nWidth = 0;
  for (int i = 0; i < m_nFixedCols; i++) nWidth += GetColumnWidth(i);
...
BOOL CGridCtrl::AutoSizeColumn(int nCol, UINT nAutoSizeStyle /*=GVS_DEFAULT*/,
  ...
  if (GetColumnWidth(nCol) <= 0) return FALSE;
  ...
  if (nAutoSizeStyle == GVS_DEFAULT) nAutoSizeStyle = GetAutoSizeStyle();

  int nStartRow = (nAutoSizeStyle & GVS_HEADER) ? 0 : GetFixedRowCount();
  int nEndRow =
      (nAutoSizeStyle & GVS_DATA) ? GetRowCount() - 1 : GetFixedRowCount() - 1;
  ...
  for (int nRow = nStartRow; nRow <= nEndRow; nRow++) {
    CGridCellBase* pCell = GetCell(nRow, nCol);
    if (pCell) size = pCell->GetCellExtent(pDC);
  ...
  ReleaseDC(pDC);
  if (bResetScroll) ResetScrollBars();
...
BOOL CGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/) {
  ...
  if (GetRowHeight(nRow) <= 0) return FALSE;
...
void CGridCtrl::AutoSizeColumns(UINT nAutoSizeStyle /*=GVS_DEFAULT*/) {
  ...
  for (int nCol = 0; nCol < nNumColumns; nCol++) {
    //  Skip hidden columns when autosizing
    if (GetColumnWidth(nCol) > 0) AutoSizeColumn(nCol, nAutoSizeStyle, FALSE);
  }
  ResetScrollBars();
}
...
void CGridCtrl::AutoSizeRows() {
  ...
  for (int nRow = 0; nRow < nNumRows; nRow++) {
    //  Skip hidden rows when autosizing
    if (GetRowHeight(nRow) > 0) AutoSizeRow(nRow, FALSE);
  }
  ResetScrollBars();
}
...
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/) {
  ...
  for (nRow = nStartRow; nRow <= nEndRow; nRow++) {
    if (GetRowHeight(nRow) > 0) m_arRowHeights[nRow] = 1;
  }
...
// Expands the columns to fit the screen space. If bExpandFixed is FALSE then
// fixed columns will not be affected
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/) {
  ...
  for (int col = nFirstColumn; col < GetColumnCount(); col++) {
    if (m_arColWidths[col] > 0) nNumColumnsAffected++;
  }

  if (nNumColumnsAffected <= 0) return;

  /////////////////////////////////////////////////////////////////////////
  // ³ÂÈ¨12ÔÂ21ÈÕÒþ
  // ¾ÉµÄ´¦Àí·½·¨£º¶ÔÊ£Óà¿Õ¼äÔÚÓÐÐ§ÁÐ¼äµÈ·Ö
  // ÐÂ´¦Àí£ºÓÐÐ§ÁÐ°´±ÈÀý·ÖÅä
  /////////////////////////////////////////////////////////////////////////
  /*
  ...
  int nDifference = rect.Width();
  int nGlobeWidth = 0;
  for (int col = nFirstColumn; col < GetColumnCount(); col++) {
    if (m_arColWidths[col] > 0) {
      nGlobeWidth += m_arColWidths[col];
    }
  }

  int leftOver = nDifference;
  for (int col = nFirstColumn; col < GetColumnCount(); col++) {
    if (m_arColWidths[col] > 0) {
      int nDelt = nDifference * m_arColWidths[col] / nGlobeWidth;
      m_arColWidths[col] = nDelt;
      leftOver -= nDelt;
    }
  }
  ...
  if (leftOver > 0) {
    for (int nCount = 0, col = nFirstColumn;
         (col < GetColumnCount()) && (nCount < leftOver); col++, nCount++) {
      if (m_arColWidths[col] > 0) m_arColWidths[col] += 1;
    }
  } else {
    for (int nCount = 0, col = nFirstColumn;
         (col < GetColumnCount()) && (nCount < leftOver); col++, nCount++) {
...
void CGridCtrl::ExpandLastColumn() {
  ...
  while (m_arColWidths[nLastColumn] <= 0) nLastColumn--;

  if (nLastColumn <= 0) return;
...
void CGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/) {
  ...
  for (int row = nFirstRow; row < GetRowCount(); row++) {
    if (m_arRowHeights[row] > 0) nNumRowsAffected++;
  }

  if (nNumRowsAffected <= 0) return;
  ...
  for (row = nFirstRow; row < GetRowCount(); row++) {
    if (m_arRowHeights[row] > 0) m_arRowHeights[row] += nRowAdjustment;
  }
  ...
  if (nDifference > 0) {
    ...
    for (int nCount = 0, row = nFirstRow;
      ...
      if (m_arRowHeights[row] > 0) m_arRowHeights[row] += 1;
    }
  } else {
    int leftOver = (-nDifference) % nNumRowsAffected;
    for (int nCount = 0, row = nFirstRow;
...
// Expands the cells to fit the screen space. If bExpandFixed is FALSE then
// fixed cells  will not be affected
void CGridCtrl::ExpandToFit(BOOL bExpandFixed /*=TRUE*/) {
  ExpandColumnsToFit(
      bExpandFixed);  // This will remove any existing horz scrollbar
  ExpandRowsToFit(
      bExpandFixed);  // This will remove any existing vert scrollbar
  ExpandColumnsToFit(bExpandFixed);  // Just in case the first adjustment was
                                     // with a vert scrollbar in place
  Refresh();
}
...
void CGridCtrl::SetSortColumn(int nCol) {
  if (m_nSortColumn >= 0) InvalidateCellRect(0, m_nSortColumn);
  m_nSortColumn = nCol;
  if (nCol >= 0) InvalidateCellRect(0, nCol);
}
...
void CGridCtrl::SetModified(BOOL bModified /*=TRUE*/, int nRow /*=-1*/,
                            int nCol /*=-1*/) {
  // Cannot guarantee sorting anymore...
  if (nCol < 0 || nCol == GetSortColumn()) SetSortColumn(-1);
...
void CGridCtrl::Refresh() {
  if (GetSafeHwnd() && m_bAllowDraw) Invalidate();
}
...
void CGridCtrl::EnsureVisible(int nRow, int nCol) {
  ...
  */
  ...
  while (down > 0) {
    if (GetRowHeight(iRowStart) > 0) SendMessage(WM_VSCROLL, SB_LINEDOWN, 0);
    down--;
    iRowStart++;
  }
  ...
  while (up > 0) {
    if (GetRowHeight(iRowStart) > 0) SendMessage(WM_VSCROLL, SB_LINEUP, 0);
    up--;
    iRowStart--;
  }
  ...
  if (pFocusWnd && ::IsWindow(pFocusWnd->GetSafeHwnd())) pFocusWnd->SetFocus();
}
...
BOOL CGridCtrl::IsCellSelected(int nRow, int nCol) const {
  if (GetVirtualMode()) {
    ...
    return IsSelectable() &&
           ((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED);
}
...
BOOL CGridCtrl::InvalidateCellRect(const int row, const int col) {
  ...
  if (!IsCellVisible(row, col)) return FALSE;

  CRect rect;
  if (!GetCellRect(row, col, rect)) return FALSE;
  rect.right++;
  rect.bottom++;
...
BOOL CGridCtrl::InvalidateCellRect(const CCellRange& cellRange) {
  ...
  CCellRange visibleCellRange =
      GetVisibleNonFixedCellRange().Intersect(cellRange);
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point) {
  ...
  if (m_MouseMode != MOUSE_DRAGGING && !rect.PtInRect(point)) return;
#endif
  ...
  // All bMouseButtonDown's have been replaced with the member
  // m_bLMouseButtonDown BOOL bMouseButtonDown = ((nFlags & MK_LBUTTON) ==
  // MK_LBUTTON);

  // If the left mouse button is up, then test to see if row/column sizing is
  // imminent
    ...
    if (m_MouseMode == MOUSE_NOTHING) {
      ...
      if (m_bTitleTips) {
        ...
        if (pCell) {
          ...
          if (!m_bRMouseButtonDown && szTipText && szTipText[0] &&
              !pCell->IsEditing() &&
            ...
            //						m_TitleTip.Show(TextRect, pCell->GetTipText(),  0,
            // CellRect,
            //                                       pCell->GetFont(),
            //                                       GetTitleTipTextClr(),
            //                                       GetTitleTipBackClr());
  ...
  if (m_bLMouseButtonDown) {
    switch (m_MouseMode) {
      ...
      case MOUSE_SIZING_COL: {
        ...
        CRect oldInvertedbfRect(m_LastMousePoint.x, rect.top,
                                m_LastMousePoint.x + 2, rect.bottom);
        pDC->InvertRect(&oldInvertedbfRect);
        CRect newInvertedbfRect(point.x, rect.top, point.x + 2, rect.bottom);
        pDC->InvertRect(&newInvertedbfRect);
        ReleaseDC(pDC);
      } break;
      ...
      case MOUSE_SIZING_ROW: {
        ...
        CRect oldInvertedbfRect(rect.left, m_LastMousePoint.y, rect.right,
                                m_LastMousePoint.y + 2);
        pDC->InvertRect(&oldInvertedbfRect);
        CRect newInvertedbfRect(rect.left, point.y, rect.right, point.y + 2);
        pDC->InvertRect(&newInvertedbfRect);
        ReleaseDC(pDC);
      } break;
...
// Returns the point inside the cell that was clicked (coords relative to cell
// top left)
CPoint CGridCtrl::GetPointClicked(int nRow, int nCol, const CPoint& point) {
  CPoint PointCellOrigin;
  if (!GetCellOrigin(nRow, nCol, &PointCellOrigin)) return CPoint(0, 0);
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) {
...
#endif
  {
    ...
    //  ignore columns that are hidden and look left towards first visible
    //  column
...
#endif
  {
    ...
    CGridCellBase* pCell = NULL;
    if (IsValid(cell)) pCell = GetCell(cell.row, cell.col);
    ...
    if (IsValid(cell)) {
      CGridCellBase* pCell = GetCell(cell.row, cell.col);
      if (pCell) pCell->OnDblClick(pointClickedRel);
...
//    m_TitleTip.Hide();  // hide any titletips
  ...
  // If the SHIFT key is not down, then the start of the selection area should
  // be the cell just clicked. Otherwise, keep the previous selection-start-cell
  // so the user can add to their previous cell selections in an intuitive way.
  // If no selection- start-cell has been specified, then set it's value here
  // and now.
  if ((nFlags & MK_SHIFT) != MK_SHIFT)
    m_SelectionStartCell = m_LeftClickDownCell;
  else {
    if (!IsValid(m_SelectionStartCell)) m_SelectionStartCell = m_idCurrentCell;
  }
  ...
  CGridCellBase* pCell =
      GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
  if (pCell)
    pCell->OnClickDown(GetPointClicked(m_LeftClickDownCell.row,
                                       m_LeftClickDownCell.col, point));

  // Clicked in the text area? Only then will cell selection work
  BOOL bInTextArea = FALSE;
  if (pCell) {
    CRect rectCell;
    if (GetCellRect(m_LeftClickDownCell.row, m_LeftClickDownCell.col,
                    rectCell) &&
        pCell->GetTextRect(rectCell)) {
      bInTextArea = rectCell.PtInRect(point);
    }
  ...
  if (m_LeftClickDownCell == m_idCurrentCell && !(nFlags & MK_CONTROL) &&
  ...
  // (If the user moves the mouse, then dragging occurs)
    ...
    // If control is pressed then unselect the cell or row (depending on the
    // list mode)
    ...
    // Don't set focus on any cell if the user clicked on a fixed cell - David
    // Pritchard if (GetRowCount() > GetFixedRowCount() &&
  ...
  if (m_MouseMode == MOUSE_OVER_COL_DIVIDE)  // sizing column
  {
    ...
    if (!m_bHiddenColUnhide) {
      ...
      if (bIsCellRightBorder &&
        ...
        if (m_LeftClickDownCell.col >= 0) bLookForVisible = FALSE;
      }
      ...
      if (bLookForVisible) {
        ...
        if (bIsCellRightBorder) iOffset = 0;
    ...
    CRect rect;
    GetClientRect(rect);
    CRect invertedbfRect(point.x, rect.top, point.x + 2, rect.bottom);

    CDC* pDC = GetDC();
    if (pDC) {
      pDC->InvertRect(&invertedbfRect);
      ReleaseDC(pDC);
    }

    // If we clicked to the right of the colimn divide, then reset the
    // click-down cell as the cell to the left of the column divide - UNLESS we
    // clicked on the last column and the last column is teensy (kludge fix)
  ...
  } else if (m_MouseMode == MOUSE_OVER_ROW_DIVIDE)  // sizing row
  {
    ...
    if (!m_bHiddenRowUnhide) {
      ...
      if (bIsCellBottomBorder && m_LeftClickDownCell.row + 1 >= GetRowCount()) {
        ...
        if (m_LeftClickDownCell.row >= 0) bLookForVisible = FALSE;
      }
      ...
      if (bLookForVisible) {
        ...
        if (bIsCellBottomBorder) iOffset = 0;
    ...
    CRect rect;
    GetClientRect(rect);
    CRect invertedbfRect(rect.left, point.y, rect.right, point.y + 2);
...
#ifndef GRIDCONTROL_NO_DRAGDROP
      if (m_MouseMode !=
          MOUSE_PREPARE_DRAG)  // not sizing or editing -- selecting
#endif
  {
    SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col,
                        GVN_SELCHANGING);
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  CPoint pointClickedRel;
  pointClickedRel =
      GetPointClicked(m_idCurrentCell.row, m_idCurrentCell.col, point);
...
#ifndef GRIDCONTROL_NO_DRAGDROP
  // m_MouseMode == MOUSE_PREPARE_DRAG only if user clicked down on a selected
  // cell and then didn't move mouse before clicking up (releasing button)
    ...
    CRect rect;
    GetClientRect(rect);
    CRect invertedbfRect(m_LastMousePoint.x, rect.top, m_LastMousePoint.x + 2,
                         rect.bottom);
    ...
    if (m_LeftClickDownPoint != point &&
        (point.x != 0 || point.y != 0))  // 0 pt fix by email1@bierling.net
    {
      CPoint start;
      if (!GetCellOrigin(m_LeftClickDownCell, &start)) return;
    ...
    CRect rect;
    GetClientRect(rect);
    CRect invertedbfRect(rect.left, m_LastMousePoint.y, rect.right,
                         m_LastMousePoint.y + 2);
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  if (!IsValid(FocusCell))
    SendMessageToParent(-1, -1, point, NM_RCLICK);
  else {
    SetFocusCell(-1, -1);
    ...
    SendMessageToParent(FocusCell.row, FocusCell.col, point, NM_RCLICK);
  }
}
#endif
...
void CGridCtrl::Print(CPrintDialog* pPrntDialog /*=NULL*/) {
  ...
  if (strTitle.IsEmpty()) {
    ...
    while (pParentWnd) {
      ...
      if (strTitle.GetLength())  // can happen if it is a CView, CChildFrm has
                                 // the title
        break;
  ...
  for (UINT page = Info.GetMinPage(); page <= Info.GetMaxPage() && bPrintingOK;
       page++) {
...
void CGridCtrl::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo) {
  ...
  m_CharSize = pDC->GetTextExtent(
      _T("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSATUVWXYZ"), 52);
  m_CharSize.cx /= 52;
  ...
  if (m_bWysiwygPrinting) {
    ...
    m_PaperSize =
        CSize(pDC->GetDeviceCaps(HORZRES), pDC->GetDeviceCaps(VERTRES));
  ...
  m_nPageHeight =
      m_LogicalPageSize.cy - GetFixedRowHeight() -
...
void CGridCtrl::OnPrint(CDC* pDC, CPrintInfo* pInfo) {
  ...
  pInfo->m_rectDraw.right =
      m_LogicalPageSize.cx - (m_nLeftMargin + m_nRightMargin) * m_CharSize.cx;
  ...
  int iWideColPageOffset =
      pInfo->m_nCurPage - (iPageIfIgnoredWideCols * m_nPageMultiplier);
  if (iWideColPageOffset > 0) iPageIfIgnoredWideCols++;

  if (iWideColPageOffset == 0) iWideColPageOffset = m_nPageMultiplier;
  iWideColPageOffset--;
  ...
  while (m_nCurrPrintRow < GetRowCount()) {
    ...
    for (int col = m_nPrintColumn; col < GetColumnCount(); col++) {
      ...
      if (rect.right > m_nPageWidth) break;

      CGridCellBase* pCell = GetCell(m_nCurrPrintRow, col);
      if (pCell) pCell->PrintCell(pDC, m_nCurrPrintRow, col, rect);
...
void CGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row,
                                   CRect& rect, CDC* pDC, BOOL& bFirst) {
  ...
  for (int col = nStartColumn; col < nStopColumn; col++) {
    ...
    if (pCell) pCell->PrintCell(pDC, row, col, rect);
...
void CGridCtrl::PrintColumnHeadings(CDC* pDC, CPrintInfo* /*pInfo*/) {
  ...
  for (int row = 0; row < GetFixedRowCount(); row++) {
    ...
    // lets printout those fixed cell headings again the 1 or more that would be
    // missed added by M.Fletcher 12/17/00
    if (m_nPrintColumn >= GetFixedColumnCount()) {
      bOriginal = bFirst;
      // lets print the missing fixed cells on left first out to last fixed
      // column
    ...
    // columns
    PrintFixedRowCells(m_nPrintColumn, GetColumnCount(), row, rect, pDC,
                       bFirst);
...
void CGridCtrl::PrintRowButtons(CDC* pDC, CPrintInfo* /*pInfo*/) {
  ...
  for (int iCol = 0; iCol < GetFixedColumnCount(); iCol++) {
    ...
    for (int iRow = m_nCurrPrintRow; iRow < GetRowCount(); iRow++) {
      ...
      if (pCell) pCell->PrintCell(pDC, iRow, iCol, rect);
...
void CGridCtrl::PrintHeader(CDC* pDC, CPrintInfo* pInfo) {
  ...
  while (pParentWnd) {
    ...
    if (strCenter.GetLength())  // can happen if it is a CView, CChildFrm has
                                // the title
      break;
    pParentWnd = pParentWnd->GetParent();
  }
  ...
  if (!strCenter.IsEmpty())
    pDC->DrawText(strCenter, &rc,
                  DT_CENTER | DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER);
  if (!strRight.IsEmpty())
    pDC->DrawText(strRight, &rc,
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator /*=_T(',')*/) {
  ...
  if (!File.Open(filename,
                 CFile::modeWrite | CFile::modeCreate | CFile::typeText, &ex)) {
    ex.ReportError();
    return FALSE;
  }
...
CImageList* CGridCtrl::CreateDragImage(CPoint* pHotSpot) {
  ...
  *pHotSpot = rect.BottomRight();
  ...
  CGridCellBase* pCell = GetCell(cell.row, cell.col);
  if (pCell) pCell->Draw(&MemDC, cell.row, cell.col, rect, FALSE);
...
void CGridCtrl::OnFixedColumnClick(CCellID& cell) {
  ...
  //    if (m_bListMode && (GetItemState(cell.row, m_nFixedCols) &
  //    GVNI_SELECTED))
  //    {
  ...
  // Did the user click on a fixed row cell (so the cell was within the overlap
  // of fixed row and column cells) - (fix by David Pritchard)
...
//    m_TitleTip.Hide();  // hide any titletips
  ...
  // Can we do it?
  CCellID cell(nRow, nCol);
  if (!IsValid(cell) || !IsCellEditable(nRow, nCol)) return;

  // Can we see what we are doing?
  EnsureVisible(nRow, nCol);
  if (!IsCellVisible(nRow, nCol)) return;
  ...
  if (SendMessageToParent(nRow, nCol, GVN_BEGINLABELEDIT) >= 0) {
    // Let's do it...
    CGridCellBase* pCell = GetCell(nRow, nCol);
    if (pCell) pCell->Edit(nRow, nCol, rect, point, IDC_INPLACE_CONTROL, nChar);
  }
}
...
int CGridCtrl::GetNextRow(int nRow, int nFlags) const {
...
LRESULT CGridCtrl::SendKeyMessage(WORD wVKey, UINT flags) const {
  if (!IsWindow(m_hWnd)) return 0;
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, POINT ptAction,
                                       int nMessage) const {
  if (!IsWindow(m_hWnd)) return 0;
...
void CGridCtrl::SetRowCol(int nFixedRow, int nFixedCol, int nRow, int nCol) {
...
void CGridCtrl::SetColWidth(int* widths) {
  for (int i = 0; i < GetColumnCount(); ++i) {
    SetColumnWidth(i, widths[i]);
  }
}

void CGridCtrl::GetSel(vector<int>& Sels) {
  Sels.clear();
  int nSel = GetNextRow(-1, 2);
  while (nSel != -1) {
    Sels.push_back(nSel);
    nSel = GetNextRow(nSel, 2);
  }
}

void CGridCtrl::SetSel(vector<int>& Sels) {
  int nFirstSel = -1;
  for (size_t i = 0; i < Sels.size(); ++i) {
    if (Sels[i] < GetRowCount()) {
      if (nFirstSel == -1) {
        nFirstSel = Sels[i];
      }
      for (int j = 0; j < GetColumnCount(); ++j) {
        SetItemState(Sels[i], j, 2);
      }
    }
  }
}
