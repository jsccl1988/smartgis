        ...
        case MOUSE_SIZING_COL:
            {
                ...
                CRect newInvertedbfRect(point.x, rect.top, 
        ...
        case MOUSE_SIZING_ROW:        
            {
                ...
                CRect oldInvertedbfRect(rect.left, m_LastMousePoint.y,
                    rect.right, m_LastMousePoint.y + 2);
                pDC->InvertRect(&oldInvertedbfRect);
                CRect newInvertedbfRect(rect.left, point.y, 
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    else if (m_MouseMode == MOUSE_SIZING_COL)
    {
        CRect rect;
        GetClientRect(rect);
        CRect invertedbfRect(m_LastMousePoint.x, rect.top, m_LastMousePoint.x + 2, rect.bottom);
    ...
    else if (m_MouseMode == MOUSE_SIZING_ROW)
    {
        CRect rect;
        GetClientRect(rect);
        CRect invertedbfRect(rect.left, m_LastMousePoint.y, rect.right, m_LastMousePoint.y + 2);
...
LRESULT CGridCtrl::SendMessageToParent(int nRow, int nCol, POINT ptAction, int nMessage) const
{
    if (!IsWindow(m_hWnd))
        return 0;
