...
#include "legacy/ui/mfc_ex/gridctrl/cell_range.h"
...
// large data #define GRIDCONTROL_NO_DRAGDROP    // Do not use OLE drag and drop
// #define GRIDCONTROL_NO_CLIPBOARD   // Do not use clipboard routines

#ifdef _WIN32_WCE
#define GRIDCONTROL_NO_TITLETIPS  // Do not use titletips for cells with large
                                  // data
...
#include "legacy/ui/mfc_ex/gridctrl/title_tip.h"
#endif

#ifndef GRIDCONTROL_NO_DRAGDROP
#include "legacy/ui/mfc_ex/gridctrl/grid_drop_target.h"
...
// Handy functions
#define IsSHIFTpressed() \
  ((GetKeyState(VK_SHIFT) & (1 << (sizeof(SHORT) * 8 - 1))) != 0)
#define IsCTRLpressed() \
...
class AFX_EXT_CLASS CGridCtrl : public CWnd {
 ...
 public:
  ...
  void SetListStyle();
  void SetRowCol(int nFixedRow, int nFixedCol, int nRow, int nCol);
  void SetColWidth(int* widths);
 ...
 public:
  ...
  int GetNextRow(int nRow, int nFlags) const;
  void GetSel(vector<int>& Sels);
  void SetSel(vector<int>& Sels);
 ...
 public:
  ...
  BOOL AutoSizeColumn(int nCol, UINT nAutoSizeStyle = GVS_DEFAULT,
...
///////////////////////////////////////////////////////////////////////////////////
 ...
 public:
  ...
  BOOL SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending,
                 LPARAM data = 0);
...
///////////////////////////////////////////////////////////////////////////////////
 ...
 public:
  ...
  void SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight, int nLeftMargin,
                          int nRightMargin, int nTopMargin, int nBottomMargin,
                          int nGap);
 ...
 public:
  ...
  /*public:
    BOOL SetRowFocusAndSelection(int nRow,int nCol=0);
    */
 protected:
  ...
  LRESULT SendMessageToParent(int nRow, int nCol, POINT ptAction,
                              int nMessage) const;
  LRESULT SendKeyMessage(WORD wVKey, UINT flags) const;
...
#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
  // Printing
  virtual void PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row,
 ...
 protected:
  ...
  // Fonts and images
  CRuntimeClass*
      m_pRtcDefault;  // determines kind of Grid Cell created by default
  CGridDefaultCell
      m_cellDefault;  // "default" cell. Contains default colours, font etc.
  ...
  int m_nHeaderHeight, m_nFooterHeight, m_nLeftMargin, m_nRightMargin,
      m_nTopMargin, m_nBottomMargin, m_nGap;

 protected:
  void SelectAllCells();
  void SelectColumns(CCellID currentCell, BOOL bForceRedraw = FALSE,
                     BOOL bSelectCells = TRUE);
  void SelectRows(CCellID currentCell, BOOL bForceRedraw = FALSE,
                  BOOL bSelectCells = TRUE);
  void SelectCells(CCellID currentCell, BOOL bForceRedraw = FALSE,
                   BOOL bSelectCells = TRUE);
...
inline CGridCellBase* CGridCtrl::GetDefaultCell(BOOL bFixedRow,
                                                BOOL bFixedCol) const {
...
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol) const {
  ...
  if (GetVirtualMode()) {
    CGridCellBase* pCell =
        GetDefaultCell(nRow < m_nFixedRows, nCol < m_nFixedCols);
