...
long SmtChart::Init() {
  ...
  if (NULL != pAnnoLyr && NULL != pPntLyr && NULL != pLineLyr &&
      NULL != pRegLyr) {
...
long SmtChart::Create() {
  ...
  if (vPts.size() < 1) return SMT_ERR_FAILURE;
...
void SmtChart::Draw() {
  ...
  //
  if (m_bShowGridLines) DrawGridLines();

  //
  if (m_bShowAxis) DrawAixs();

  //
  DrawData();
}

void SmtChart::SetPoints(const vPoints &points) { m_cData.m_vPoints = points; }
...
void SmtChart::DrawGridLines(void) {
  ...
  pSmtFeature->SetFieldValue(pSmtFeature->GetFieldIndexByName("length"),
                             ((OGRCurve *)pMLineString)->get_Length());
  pSmtFeature->SetID(pLineLayer->GetFeatureCount() + 1);
  leftover_append_feature(pLineLayer, pSmtFeature);
}
...
void SmtChart::DrawAixs(void) {
  ...
  point.x =
      m_cPanel.rtContent.lb.x + (m_cPanel.rtContent.width() - fStrWidth) / 2;
  ...
  pSmtFeature->SetFieldValue(pSmtFeature->GetFieldIndexByName("length"),
                             ((OGRCurve *)pLineString)->get_Length());
  pSmtFeature->SetID(pLineLayer->GetFeatureCount() + 1);
  leftover_append_feature(pLineLayer, pSmtFeature);
  ...
  for (int i = 0; i < nWidth; i++) {
    if (is_equal(int(point.x / 50.) * 50, point.x, dEPSILON)) {
      ...
      pSmtFeature->SetFieldValue(pSmtFeature->GetFieldIndexByName("color"),
                                 int(RGB(0, 0, 0)));
      pSmtFeature->SetFieldValue(pSmtFeature->GetFieldIndexByName("angle"),
                                 PI / 2.);
      pSmtFeature->SetID(pAnnoLayer->GetFeatureCount() + 1);
      leftover_append_feature(pAnnoLayer, pSmtFeature);
    }
