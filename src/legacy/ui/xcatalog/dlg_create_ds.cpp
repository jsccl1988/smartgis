...
ON_EN_KILLFOCUS(IDC_EDIT_DS_DBSEVEICE,
                &CDlgCreateDS::OnEnKillfocusEditDsDbseveice)
...
ON_NOTIFY(TVN_SELCHANGED, IDC_DSTYPE_TREE,
          &CDlgCreateDS::OnTvnSelchangedDstypeTree)
ON_BN_CLICKED(IDC_BTN_SELPATH, &CDlgCreateDS::OnBnClickedBtnSelpath)
ON_EN_KILLFOCUS(IDC_EDIT_DS_FILE_PATH,
                &CDlgCreateDS::OnEnKillfocusEditDsFilePath)
END_MESSAGE_MAP()
...
void CDlgCreateDS::OnNMClickDstypeTree(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  *pResult = 0;
  ...
  if (hParentItem == m_hTypeRoot) {
    ...
    if (m_strSelTypeName == "���ݿ�") {
      m_unType = eDSType::DS_DB_ADO;
    } else if (m_strSelTypeName == "�ļ�") {
      m_unType = eDSType::DS_FILE_SMF;
    }

    UpdateProviderCmb();

    if (m_strSelTypeName == "�ļ�") {
...
void CDlgCreateDS::UpdateDSTypeTree(void) {
  ...
  m_hTypeRoot = m_typeTree.InsertItem("����Դ����");

  m_typeTree.InsertItem("���ݿ�", m_hTypeRoot);
  m_typeTree.InsertItem("�ļ�", m_hTypeRoot);
...
void CDlgCreateDS::OnBnClickedBtnSelpath() {
  ...
  sprintf(szFilter, "Data Files (*.%s)|*.%s|All Files (*.*)|*.*||", "smf",
          "smf");
...
void CDlgCreateDS::OnBnClickedBtnSeldb() {
  // TODO: �ڴ����ӿؼ�֪ͨ�����������
  static char szFilter[256];
  sprintf(szFilter, "Data Files (*.%s)|*.%s|All Files (*.*)|*.*||", "mdb",
          "mdb");
