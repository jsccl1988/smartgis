...
void SmtMapMgr::DestoryInstance(void) { SMT_SAFE_DELETE(m_pSingleton); }
...
bool SmtMapMgr::OpenMap(SmtMap *pMap, const char *szMapFile) {
  ...
  if (stricmp(strExt.c_str(), "mdoc") != 0) return false;
  ...
  // Binary mdoc I/O; avoid locale(".936") (can throw) and ios::out on
  // ifstream (truncates / fails to load layers → empty map).
  ifstream infile;
  infile.open(strFileName.c_str(), ios::in | ios::binary);
...
bool SmtMapMgr::OpenMap(const char *szMapFile) {
  ...
  // Binary mdoc I/O; avoid locale(".936") and ios::out on ifstream.
  ifstream infile;
  infile.open(m_strMDocPath.c_str(), ios::in | ios::binary);
...
bool SmtMapMgr::CloseMap() {
  if (NULL != m_pSmtMap && ::MessageBox(NULL, "Save Map Document?",
                                        "SmartGis - Tip", MB_YESNO) == IDYES) {
    SaveMap();
  }
...
bool SmtMapMgr::SaveMap() {
  if (NULL == m_pSmtMap) return false;
  ...
  if (m_strMDocPath == "") {
    ...
    sprintf_s(szBuf, _MAX_DIR, "%sdata source\\%s.mdoc", szParentPath,
              m_pSmtMap->GetMapName());
...
bool SmtMapMgr::Update2DXView(void) {
  if (m_fn2DXViewNotify == NULL) return true;
...
bool SmtMapMgr::AppendLayer(OGRLayer *pLayer) {
  ...
  if (m_pSmtMap) {
    LOGGING(LOG_INFO, "AppendLayer(OGR): AddLayer begin map=%p lyr=%p",
            m_pSmtMap, pLayer);
    bRet = m_pSmtMap->AddLayer(pLayer);
    LOGGING(LOG_INFO, "AppendLayer(OGR): AddLayer done bRet=%d", bRet);
    m_pSmtMap->SetActiveOgrLayer(pLayer);
    LOGGING(LOG_INFO, "AppendLayer(OGR): UpdateMapCatalog begin cats=%zu",
            m_vMapCatalogPtrs.size());
    UpdateMapCatalog();
    LOGGING(LOG_INFO, "AppendLayer(OGR): UpdateMapCatalog done");
  }
