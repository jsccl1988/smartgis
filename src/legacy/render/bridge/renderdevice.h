...
class SmtRenderDevice {
 ...
 public:
  ...
  virtual int RenderMap(const SmtMap *pMap,
                        int op = R2_COPYPEN) = 0;  // 1-16 R2_BLACK-R2_WHITE
  virtual int RenderLayer(const SmtLayer *pLayer,
                          int op = R2_COPYPEN) = 0;  // 1-16 R2_BLACK-R2_WHITE
  virtual int RenderLayer(OGRLayer *pLayer, int op = R2_COPYPEN) = 0;
  virtual int RenderLayer(const SmtRasterLayer *pLayer,
                          int op = R2_COPYPEN) = 0;  // 1-16 R2_BLACK-R2_WHITE
  virtual int RenderLayer(const SmtTileLayer *pLayer,
                          int op = R2_COPYPEN) = 0;  // 1-16 R2_BLACK-R2_WHITE
...
extern "C" {
#endif
int RENDER_EXPORT_API CreateRenderDevice(HINSTANCE hInst,
