...
namespace render {
SmtSphere::SmtSphere(float radius, DWORD slices)
    : m_dwSlices(slices),
      m_fRadius(radius),
...
SmtSphere::~SmtSphere() { Destroy(); }

long SmtSphere::Init(Vector3& vPos, SmtMaterial& matMaterial,
...
long SmtSphere::Create(LP3DRENDERDEVICE p3DRenderDevice) {
  ...
  m_pVertexBuffer = p3DRenderDevice->CreateVertexBuffer(
      2 * (m_dwSlices + 1) * m_dwSlices, VF_XYZ | VF_TEXCOORD | VF_NORMAL,
      false);
  ...
  m_aAbb.vcMax.set(m_fRadius * m_fXScale, m_fRadius * m_fYScale,
                   m_fRadius * m_fZScale);
  m_aAbb.vcMin.set(-m_fRadius * m_fXScale, -m_fRadius * m_fYScale,
                   -m_fRadius * m_fZScale);
