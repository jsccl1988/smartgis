...
namespace render {
	...
	SmtGdiSimpleRenderDevice::~SmtGdiSimpleRenderDevice(void)
	{
       Release();
	}
	...
	int SmtGdiSimpleRenderDevice::LRectToDRect(const fRect &frect,lRect &lrect) const 
	{
	...
	int SmtGdiSimpleRenderDevice::DRectToLRect(const lRect &lrect,fRect &frect) const 
	{
	...
	int SmtGdiSimpleRenderDevice::RefreshDirectly(const SmtMap *pSmtMap,lRect rect,bool bRealTime )
	{
		if (pSmtMap)
		{
			return RenderMap(pSmtMap,m_Viewport.m_fVOX,m_Viewport.m_fVOY,m_Viewport.m_fVWidth,m_Viewport.m_fVHeight);
		}

		return SMT_ERR_INVALID_PARAM;
	}

	int SmtGdiSimpleRenderDevice::ZoomMove(const SmtMap *pSmtMap,fPoint dbfPointOffset,bool bRealTime)
	{ 
	...
	int SmtGdiSimpleRenderDevice::ZoomToRect(const SmtMap *pSmtMap,fRect rect,bool bRealTime)
	{
		lRect rt;
			...
			{//�����ػ�
				...
				if (m_pCurMap != NULL)
				{
					for (int i = 0; i < m_pCurMap->GetLayerCount(); ++i) {
						...
						if (OGRLayer* ogr = const_cast<OGRLayer*>(
								m_pCurMap->GetOgrLayer(i))) {
	...
	//////////////////////////////////////////////////////////////////////////
	int SmtGdiSimpleRenderDevice::BeginRender(eRDBufferLayer eMRDBufLyr,bool bClear,const SmtStyle*pStyle,int op)
	{
	...
	int SmtGdiSimpleRenderDevice::PrepareForDrawing(const SmtStyle*pStyle,int nDrawMode)
	{
	...
	int SmtGdiSimpleRenderDevice::RenderMap( const SmtMap *pMap,int x,int y,int w,int h,int op)
	{
		if ( w == 0 || h == 0 ) 
			return SMT_ERR_INVALID_PARAM;
	...
	int SmtGdiSimpleRenderDevice::RenderLayer(const SmtRasterLayer *pLayer,int op)
	{
		if (NULL == pLayer)
			return SMT_ERR_INVALID_PARAM;
	...
	//////////////////////////////////////////////////////////////////////////
	int SmtGdiSimpleRenderDevice::DrawEllipse(float left,float top,float right,float bottom,bool bDP )
	{
		lRect lrect;
	...
	int SmtGdiSimpleRenderDevice::DrawLine(const fPoint &ptA,const fPoint &ptB,bool bDP)
	{
	...
	int SmtGdiSimpleRenderDevice::DrawText(const char *szAnno,float fangel,float fCHeight,float fCWidth,float fCSpace,const fPoint &point,bool bDP)
	{
		if (szAnno == NULL)
			return SMT_ERR_INVALID_PARAM;
	...
	int SmtGdiSimpleRenderDevice::DrawImage(const char *szImageBuf,int nImageBufSize,const fRect &frect,long lCodeType,eRDBufferLayer eMRDBufLyr)
	{
	...
	int SmtGdiSimpleRenderDevice::StrethImage(const char *szImageBuf,int nImageBufSize,const fRect &frect,long lCodeType,eRDBufferLayer eMRDBufLyr)
	{
		long lRtn = SMT_ERR_FAILURE;
	...
	int SmtGdiSimpleRenderDevice::SaveImage(const char * szFilePath,eRDBufferLayer eMRDBufLyr,bool bBgTransparent)
	{
		long lRtn = SMT_ERR_FAILURE;
	...
	int SmtGdiSimpleRenderDevice::Save2ImageBuf(char *&szImageBuf,long &lImageBufSize,long lCodeType,eRDBufferLayer eMRDBufLyr,bool bBgTransparent)
	{
		long lRtn = SMT_ERR_FAILURE;
