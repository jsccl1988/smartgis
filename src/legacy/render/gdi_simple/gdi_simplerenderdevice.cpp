#include "legacy/render/gdi_simple/gdi_simplerenderdevice.h"
...
//////////////////////////////////////////////////////////////////////////
SmtGdiSimpleRenderDevice::SmtGdiSimpleRenderDevice(HINSTANCE hInst)
...
SmtGdiSimpleRenderDevice::~SmtGdiSimpleRenderDevice(void) { Release(); }

int SmtGdiSimpleRenderDevice::Init(HWND hWnd, const char *logname) {
  if (hWnd == NULL || logname == NULL) return SMT_ERR_INVALID_PARAM;

  m_hWnd = hWnd;
  bind_rhi_present(hWnd);

  LOGGING(LOG_INFO, "Init Gdi SmtSimpleRenderDevice ok!");
...
int SmtGdiSimpleRenderDevice::Destroy(void) {
  LOGGING(LOG_INFO, "Destroy Gdi SmtSimpleRenderDevice ok!");

  return SMT_ERR_NONE;
}

int SmtGdiSimpleRenderDevice::Release(void) {
  LOGGING(LOG_INFO, "Release Gdi SmtSimpleRenderDevice ok!");
...
int SmtGdiSimpleRenderDevice::Resize(int orgx, int orgy, int cx, int cy) {
  ...
  if (SMT_ERR_NONE == m_smtRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
                                                m_Viewport.m_fVHeight) &&
      SMT_ERR_NONE == m_smtMapRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
                                                   m_Viewport.m_fVHeight)) {
    if (SMT_ERR_NONE ==
        ...
        SMT_ERR_NONE == m_smtMapRenderBuf.SwapBuf(
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                            m_Viewport.m_fVWidth, m_Viewport.m_fVHeight,
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY)) {
      return SMT_ERR_NONE;
...
int SmtGdiSimpleRenderDevice::LRectToDRect(const fRect &frect,
                                           lRect &lrect) const {
...
int SmtGdiSimpleRenderDevice::DRectToLRect(const lRect &lrect,
                                           fRect &frect) const {
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::Lock() {
  m_cslock.lock();
  return SMT_ERR_NONE;
}

int SmtGdiSimpleRenderDevice::Unlock() {
  m_cslock.unlock();
  return SMT_ERR_NONE;
}

int SmtGdiSimpleRenderDevice::Refresh() {
  //
  ...
  clear_rect(hDC, invalidatex1, invalidatey1, invalidatew1,
             invalidateh1 /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
  clear_rect(hDC, invalidatex2, invalidatey2, invalidatew2,
...
int SmtGdiSimpleRenderDevice::Refresh(const SmtMap *pMap, fRect frect) {
  lRect lrect;
...
int SmtGdiSimpleRenderDevice::RefreshDirectly(const SmtMap *pSmtMap, lRect rect,
                                              bool bRealTime) {
  if (pSmtMap) {
    return RenderMap(pSmtMap, m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                     m_Viewport.m_fVWidth, m_Viewport.m_fVHeight);
  }
...
int SmtGdiSimpleRenderDevice::ZoomMove(const SmtMap *pSmtMap,
                                       fPoint dbfPointOffset, bool bRealTime) {
...
int SmtGdiSimpleRenderDevice::ZoomScale(const SmtMap *pSmtMap, lPoint orgPoint,
                                        float fscale, bool bRealTime) {
  if (fscale > 1.) {  // �ı�m_virViewport1
...
int SmtGdiSimpleRenderDevice::ZoomToRect(const SmtMap *pSmtMap, fRect rect,
                                         bool bRealTime) {
  lRect rt;
...
int SmtGdiSimpleRenderDevice::Timer() {
  if (m_bRedraw) {
    double dbfElapse = 0.;
    ...
    if (dbfElapse > C_fDELAY) {  // �����ػ�
      ...
      if (m_pCurMap != NULL) {
        for (int i = 0; i < m_pCurMap->GetLayerCount(); ++i) {
          if (!m_pCurMap->IsLayerVisible(i)) {
            continue;
          }
          if (OGRLayer *ogr =
                  const_cast<OGRLayer *>(m_pCurMap->GetOgrLayer(i))) {
            RenderLayer(ogr, m_nOp);
          } else {
            RenderLayer(m_pCurMap->GetLeftoverLayer(i), m_nOp);
          }
        }
      }
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::BeginRender(eRDBufferLayer eMRDBufLyr,
                                          bool bClear, const SmtStyle *pStyle,
                                          int op) {
...
int SmtGdiSimpleRenderDevice::EndRender(eRDBufferLayer eMRDBufLyr) {
...
int SmtGdiSimpleRenderDevice::PrepareForDrawing(const SmtStyle *pStyle,
                                                int nDrawMode) {
...
int SmtGdiSimpleRenderDevice::EndDrawing() {
  if (m_bCurUseStyle) {
...
int SmtGdiSimpleRenderDevice::RenderMap(void) {
  m_smtRenderBuf.ClearBuf(
      m_Viewport.m_fVOX, m_Viewport.m_fVOY, m_Viewport.m_fVWidth,
      m_Viewport.m_fVHeight /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
...
int SmtGdiSimpleRenderDevice::RenderMap(const SmtMap *pMap, int x, int y, int w,
                                        int h, int op) {
...
int SmtGdiSimpleRenderDevice::RenderMap(const SmtMap *pMap, int op) {
  if (NULL == pMap) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::RenderLayer(const SmtLayer *pLayer, int op) {
  if (NULL == pLayer) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::RenderLayer(OGRLayer *pLayer, int op) {
  if (NULL == pLayer) return SMT_ERR_INVALID_PARAM;
  ...
  if (!envLayer.intersects(envViewp)) return SMT_ERR_NONE;
...
int SmtGdiSimpleRenderDevice::RenderLayer(const SmtRasterLayer *pLayer,
                                          int op) {
  if (NULL == pLayer) return SMT_ERR_INVALID_PARAM;

  if (!pLayer->IsVisible()) return SMT_ERR_NONE;
  ...
  if (SMT_ERR_NONE == pLayer->GetRasterNoClone(pRasterBuf, lRasterBufSize,
                                               locRect, lCodeType)) {
    lRect lrt;
...
int SmtGdiSimpleRenderDevice::RenderLayer(const SmtTileLayer *pLayer, int op) {
  if (NULL == pLayer) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::RenderFeature(OGRFeature *pFeature, int op) {
  if (NULL == pFeature) return SMT_ERR_INVALID_PARAM;

  m_nFeatureType = gis::datasource::infer_feature_type(
      pFeature, SmtFeatureType::SmtFtUnknown);
  SmtStyle *pStyle = gis::datasource::copy_ogr_style_from_ogr(pFeature);
  const bool owned_style = pStyle != nullptr;
  SmtStyle fallback;
  if (!pStyle) {
    gis::datasource::fill_default_draw_style(pFeature, &fallback, m_fblc);
    pStyle = &fallback;
  }
  OGRGeometry *pGeom = gis::datasource::decode_ogr_geometry(
...
int SmtGdiSimpleRenderDevice::RenderGeometry(const OGRGeometry *pGeom,
                                             const SmtStyle *pStyle, int op) {
  if (!pGeom) return SMT_ERR_INVALID_PARAM;
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawMultiLineString(
...
int SmtGdiSimpleRenderDevice::DrawMultiPoint(const SmtStyle *pStyle,
...
int SmtGdiSimpleRenderDevice::DrawMultiPolygon(
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawPoint(const SmtStyle *pStyle,
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawAnno(const char *szAnno, float fangel,
                                       float fCHeight, float fCWidth,
                                       float fCSpace, const OGRPoint *pPoint) {
  if (szAnno == NULL || !pPoint) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::DrawSymbol(HICON hIcon, long lHeight, long lWidth,
                                         const OGRPoint *pPoint) {
  lHeight *= m_fblc;
  lWidth *= m_fblc;
...
int SmtGdiSimpleRenderDevice::DrawPloygon(const OGRPolygon *pPloygon) {
  ...
  if (nExteriorPts < 2) return SMT_ERR_INVALID_PARAM;
  ...
  if (m_rdPra.bShowPoint) {
    ...
    for (int i = 0; i < nInteriorRings; i++) {
      ...
      for (int j = 0; j < nInteriorPts; ++j, nCount++) {
        ...
        LPToDP(pInteriorRing->getX(j), pInteriorRing->getY(j),
               lpPoint[nCount].x, lpPoint[nCount].y);
        // Ellipse(m_hCurDC,lpPoint[i].x - r ,lpPoint[i].y - r,lpPoint[i].x + r
        // ,lpPoint[i].y + r); Rectangle(m_hCurDC,lpPoint[i].x - r ,lpPoint[i].y
        // - r,lpPoint[i].x + r ,lpPoint[i].y + r);
        draw_cross(m_hCurDC, lpPoint[i].x, lpPoint[i].y, r);
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawTin(const SmtTin *pTin) {
  DrawTinLines(pTin);
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawGrid(const SmtGrid *pGrid) {
  DrawGridLines(pGrid);
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiSimpleRenderDevice::DrawEllipse(float left, float top, float right,
...
int SmtGdiSimpleRenderDevice::DrawRect(const fRect &rect, bool bDP) {
  lRect tmpRectDP;
...
int SmtGdiSimpleRenderDevice::DrawLine(fPoint *pfPoints, int nCount, bool bDP) {
  int nPoints = nCount;
  if (nPoints < 2) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::DrawLine(const fPoint &ptA, const fPoint &ptB,
                                       bool bDP) {
...
int SmtGdiSimpleRenderDevice::DrawText(const char *szAnno, float fangel,
                                       float fCHeight, float fCWidth,
                                       float fCSpace, const fPoint &point,
                                       bool bDP) {
  if (szAnno == NULL) return SMT_ERR_INVALID_PARAM;
...
int SmtGdiSimpleRenderDevice::DrawImage(const char *szImageBuf,
                                        int nImageBufSize, const fRect &frect,
                                        long lCodeType,
...
int SmtGdiSimpleRenderDevice::StrethImage(const char *szImageBuf,
                                          int nImageBufSize, const fRect &frect,
                                          long lCodeType,
                                          eRDBufferLayer eMRDBufLyr) {
  long lRtn = SMT_ERR_FAILURE;
...
int SmtGdiSimpleRenderDevice::SaveImage(const char *szFilePath,
                                        eRDBufferLayer eMRDBufLyr,
                                        bool bBgTransparent) {
  long lRtn = SMT_ERR_FAILURE;
...
int SmtGdiSimpleRenderDevice::Save2ImageBuf(char *&szImageBuf,
...
int SmtGdiSimpleRenderDevice::FreeImageBuf(char *&szImageBuf) {
