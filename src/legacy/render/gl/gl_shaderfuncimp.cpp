#include "legacy/render/gl/gl_shaderfuncimp.h"
...
GLuint SmtShadersFuncImpl::glCreateProgram() { return _glCreateProgram(); }
...
void SmtShadersFuncImpl::glGetObjectParameteriv(GLhandleARB programId,
...
void SmtShadersFuncImpl::glShaderSource(
...
void SmtShadersFuncImpl::glDeleteObject(
    GLhandleARB handle)  // TODO: Is it really about shaders?
{
  _glDeleteObject(handle);
}

void SmtShadersFuncImpl::glGetInfoLog(GLhandleARB programId, GLsizei size,
                                      GLsizei *written, GLcharARB *log) {
  _glGetInfoLog(programId, size, written, log);
}

GLint SmtShadersFuncImpl::glGetUniformLocation(GLuint shaderId,
                                               const GLchar *name) {
  return _glGetUniformLocation(shaderId, name);
}

void SmtShadersFuncImpl::glUniform1fv(GLint paramId, GLsizei size,
                                      const GLfloat *value) {
  _glUniform1fv(paramId, size, value);
}

void SmtShadersFuncImpl::glUniform2fv(GLint paramId, GLsizei size,
                                      const GLfloat *value) {
  _glUniform2fv(paramId, size, value);
}

void SmtShadersFuncImpl::glUniform3fv(GLint paramId, GLsizei size,
                                      const GLfloat *value) {
  _glUniform3fv(paramId, size, value);
}

void SmtShadersFuncImpl::glUniform4fv(GLint paramId, GLsizei size,
                                      const GLfloat *value) {
  _glUniform4fv(paramId, size, value);
}

void SmtShadersFuncImpl::glUniform4f(GLint paramId, float a, float b, float c,
                                     float d) {
...
void SmtShadersFuncImpl::glGetUniformfv(GLuint shaderId, GLint paramId,
                                        GLfloat *values) {
  _glGetUniformfv(shaderId, paramId, values);
}

void SmtShadersFuncImpl::glGetUniformiv(GLuint shaderId, GLint paramId,
                                        GLint *values) {
