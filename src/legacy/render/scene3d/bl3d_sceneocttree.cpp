...
SmtSceneOctTree::SmtSceneOctTree()
    : m_pRootNode(NULL), m_nAllRenderTargetsNum(0), m_bShowNodeBox(true) {}

SmtSceneOctTree::~SmtSceneOctTree() { DestroyTree(); }
...
long SmtSceneOctTree::CreateOctTree(vSmt3DObjectPtrs &v3DObjectPtrs) {
  ...
  m_pRootNode->fWidth = max(m_aabbScene.vcMax.x - m_aabbScene.vcMin.x,
                            m_aabbScene.vcMax.y - m_aabbScene.vcMin.y);
  m_pRootNode->fWidth =
      max(m_aabbScene.vcMax.z - m_aabbScene.vcMin.z, m_pRootNode->fWidth);
  ...
  m_pRootNode->CreateNode(v3DObjectPtrs, v3DObjectPtrs.size(),
