...
//////////////////////////////////////////////////////////////////////////
long SmtVertexOctTreeNode::CreateNode(SmtVertex3DList &lstVers, Vector3 vCenter,
                                      byte octCode, double width,
                                      LP3DRENDERDEVICE p3DRenderDevice) {
  this->fWidth = width;
  this->vCenterPos = vCenter;
  this->unOctCode = octCode;
  if ((lstVers.nCount > g_nMdlMaxTargets) &&
      (g_nMdlCurrentSubdivision < g_nMdlMaxSubdivision)) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      Aabb aabbSubNode(
          Vector3(vSubCenter.x - fWidth / 4, vSubCenter.y - fWidth / 4,
      ...
      for (int i = 0; i < lstVers.nCount; i++) {
        if (!vBDealedTarget[i] &&
            aabbSubNode.contains(lstVers.pVertexs[i].ver)) {
          vBInsideNode[i] = true;
    ...
    for (int i = 0; i < 8; i++) {
      vector<bool> &vBInsideNode = vvbInsideNode[i];
      this->CreateSubNode(this, pSubNodes[i], lstVers, vBInsideNode,
                          nTargetNum[i], i, p3DRenderDevice);
    }
  } else {
    ...
    this->pVertexBuffer = p3DRenderDevice->CreateVertexBuffer(
        this->vertexList.nCount, VF_XYZ | VF_DIFFUSE, false);
...
void SmtVertexOctTreeNode::CreateSubNode(SmtVertexOctTreeNode *pParent,
                                         SmtVertexOctTreeNode *&pSub,
                                         SmtVertex3DList &lstVers,
  ...
  if (nTargets > 0) {
    ...
    pSub->CreateNode(verTmpList, pParent->GetSubNodeCenter(nSubID),
                     pParent->GetSubNodeCode(nSubID), pParent->fWidth / 2.,
                     p3DRenderDevice);
    g_nMdlCurrentSubdivision--;
...
void SmtVertexOctTreeNode::RenderNodeObject(LP3DRENDERDEVICE p3DRenderDevice,
  ...
  if (smtFrustum.IsCubeIn(vCenterPos.x, vCenterPos.y, vCenterPos.z,
    ...
    if (bSubDivided) {
      ...
      if (NULL != pVertexBuffer) {
        p3DRenderDevice->DrawPrimitives(PT_POINTLIST, pVertexBuffer, 0,
...
int SmtVertexOctTreeNode::GetSubDepth() {
  if (bSubDivided) {
    int nDepth = 0;
    for (int i = 0; i < 8; i++) {
      if (pSubNodes[i]) nDepth = max(pSubNodes[i]->GetSubDepth(), nDepth);
    }
    return (++nDepth);
  } else
    return 1;
...
// Ѱ�ҵ����ڵ���С��Χ�����ڽڵ�ָ��
SmtVertexOctTreeNode *SmtVertexOctTreeNode::FindMinBoxOctNode(
    const Vector3 &point) {
