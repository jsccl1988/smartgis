#include "legacy/render/scene3d/bl3d_sceneocttree.h"
...
//////////////////////////////////////////////////////////////////////////
long SmtSceneOctTreeNode::CreateNode(vSmt3DObjectPtrs &v3DObjectPtrs,
                                     int nTarget, Vector3 vCenter,
                                     float width) {
  fWidth = width;
  vCenterPos = vCenter;
  if ((nTarget > g_nSceneMaxTargets) &&
      (g_nSceneCurrentSubdivision < g_nSceneMaxSubdivision)) {
    bSubDivided = true;
    ...
    for (int i = 0; i < 8; i++) {
      ...
      Aabb aabbSubNode(
          Vector4(vSubCenter.x - fWidth / 4, vSubCenter.y - fWidth / 4,
                  vSubCenter.z - fWidth / 4),
          Vector4(vSubCenter.x + fWidth / 4, vSubCenter.y + fWidth / 4,
                  vSubCenter.z + fWidth / 4));
      ...
      while (iter != v3DObjectPtrs.end()) {
        if (NULL != (*iter)) {
          if (!vBDealedTarget[iTargetIndex] &&
              aabbSubNode.contains((*iter)->GetAabb().vcCenter)) {
    ...
    for (int i = 0; i < 8; i++) {
      vector<bool> &vBInsideNode = vvbInsideNode[i];
      this->CreateSubNode(this, pSubNodes[i], v3DObjectPtrs, vBInsideNode,
                          nTargetNum[i], i);
    }
  } else {
...
Vector3 SmtSceneOctTreeNode::GetSubNodeCenter(int nSubID) {
  ...
  switch (nSubID) {
    case TOP_LEFT_FRONT:
      vNodeCenter = Vector3(vCtr.x - fWidth / 4, vCtr.y + fWidth / 4,
                            vCtr.z + fWidth / 4);
      break;
...
void SmtSceneOctTreeNode::CreateSubNode(SmtSceneOctTreeNode *pParent,
                                        SmtSceneOctTreeNode *&pSub,
                                        vSmt3DObjectPtrs &v3DObjectPtrs,
                                        vector<bool> vbInSubNode, int nTargets,
                                        int nSubID) {
  if (nTargets > 0) {
    ...
    pSub->CreateNode(vTmpTargetPtrs, nTargets,
                     pParent->GetSubNodeCenter(nSubID), pParent->fWidth / 2.);
    g_nSceneCurrentSubdivision--;

    vTmpTargetPtrs.clear();
  }
}
void SmtSceneOctTreeNode::UpdateNodeObject(LP3DRENDERDEVICE p3DRenderDevice,
                                           float fElapsed) {
  // �����ӽڵ�
  if (bSubDivided) {
    for (int i = 0; i < 8; i++) {
...
void SmtSceneOctTreeNode::RenderNodeObject(LP3DRENDERDEVICE p3DRenderDevice,
                                           SmtFrustum &smtFrustum,
                                           bool bShowOctNodeBox) {
  if (smtFrustum.IsCubeIn(vCenterPos.x, vCenterPos.y, vCenterPos.z,
                          fWidth / 2.)) {
    // ��Ⱦ���ڵ�ڵ�
    if (bShowOctNodeBox) {
    ...
    if (bSubDivided) {
      for (int i = 0; i < 8; i++) {
        if (pSubNodes[i])
          pSubNodes[i]->RenderNodeObject(p3DRenderDevice, smtFrustum,
                                         bShowOctNodeBox);
      }
    } else {
      // ��Ⱦ���ڵ����
...
void SmtSceneOctTreeNode::SelectNodeObject(vSmt3DObjectPtrs &vSelected3DObjects,
  ...
  if (smtFrustum.IsCubeIn(vCenterPos.x, vCenterPos.y, vCenterPos.z,
    ...
    if (bSubDivided) {
      for (int i = 0; i < 8; i++) {
        if (pSubNodes[i])
          pSubNodes[i]->SelectNodeObject(vSelected3DObjects, p3DRenderDevice,
                                         smtFrustum, point);
      }
    } else {
      // ʰȡ���ڵ����
...
bool SmtSceneOctTreeNode::IsInOctNodeAabbBox(const Vector3 &point) {
  Aabb aabbNode(Vector3(this->vCenterPos.x - this->fWidth / 2,
                        this->vCenterPos.y - this->fWidth / 2,
                        this->vCenterPos.z - this->fWidth / 2),
                Vector3(this->vCenterPos.x + this->fWidth / 2,
                        this->vCenterPos.y + this->fWidth / 2,
                        this->vCenterPos.z + this->fWidth / 2));
...
SmtSceneOctTreeNode *SmtSceneOctTreeNode::FindMinBoxOctNode(const Ray &ray) {
  ...
  if (!ray.intersects(aabbNode, &f)) return NULL;
...
SmtSceneOctTreeNode *SmtSceneOctTreeNode::FindMinBoxOctNode(
