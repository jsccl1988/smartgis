...
namespace render {
SmtTexture::SmtTexture(LP3DRENDERDEVICE p3DRenderDevice, uint handle,
                       string strName)
    : m_unHandle(handle),
...
TextureSampler SmtTexture::GetSampler() const { return m_texSampler; }

void SmtTexture::SetSampler(TextureSampler sampler) { m_texSampler = sampler; }

TextureEnvMode SmtTexture::GetEnvMode() const { return m_texEnv; }

void SmtTexture::SetEnvMode(TextureEnvMode envMode) { m_texEnv = envMode; }

TextureDesc SmtTexture::GetDesc() const { return m_texDesc; }
...
long SmtTexture::Create(ulong ulWidth, ulong ulHeight, TextureFormat format,
...
void *SmtTexture::GetData() { return m_pBuffer; }
