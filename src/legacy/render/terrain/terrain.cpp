...
#include "legacy/render/render3d/texturemanager.h"
...
long SmtTerrain::Init(Vector3 &vPos, SmtMaterial &matMaterial,
  ...
  m_pClr[0].fRed = 255 / 255.;
  m_pClr[0].fGreen = 0. / 255.;
  m_pClr[0].fBlue = 0. / 255.;
  m_pClr[0].fA = 1.0;
  m_pClr[1].fRed = 0. / 255.;
  m_pClr[1].fGreen = 255. / 255.;
  m_pClr[1].fBlue = 0. / 255.;
  m_pClr[0].fA = 1.0;
  m_pClr[2].fRed = 0. / 255.;
  m_pClr[2].fGreen = 0. / 255.;
  m_pClr[2].fBlue = 255. / 255.;
  m_pClr[0].fA = 1.0;
...
long SmtTerrain::Create(LP3DRENDERDEVICE p3DRenderDevice) {
  ...
  m_pVertexBuffer = p3DRenderDevice->CreateVertexBuffer(
      m_p3DSurf->get_point_count(),
      VF_XYZ | VF_TEXCOORD | VF_NORMAL | VF_DIFFUSE, false);

  m_pVertexBuffer->Lock();

  for (int i = 0; i < m_p3DSurf->get_point_count(); i++) {
    OGRPoint point = m_p3DSurf->get_point(i);
    m_pVertexBuffer->Vertex(point.getX(), point.getZ(), point.getY());
  }

  m_pIndexBuffer =
      p3DRenderDevice->CreateIndexBuffer(m_p3DSurf->get_triangle_count() * 3);
  m_pIndexBuffer->Lock();

  for (int i = 0; i < m_p3DSurf->get_triangle_count(); i++) {
    Smt3DTriangle tri = m_p3DSurf->get_triangle(i);
...
long SmtTerrain::Render(LP3DRENDERDEVICE p3DRenderDevice) {
  if (NULL == p3DRenderDevice) return SMT_ERR_INVALID_PARAM;
...
void SmtTerrain::RenderTerrain(LP3DRENDERDEVICE p3DRenderDevice) {
  ...
  p3DRenderDevice->DrawIndexedPrimitives(PT_TRIANGLELIST, m_pVertexBuffer,
                                         m_pIndexBuffer, 0,
                                         m_pIndexBuffer->GetIndexCount() / 3);
...
void SmtTerrain::GetColor(float h, SmtColor &clr) {
  ...
  if (m_nClrType == 1) {
    ...
    if (h <= m_midHeight) {
      ...
      clr.fGreen =
          (m_pClr[1].fGreen - m_pClr[0].fGreen) * hs + m_pClr[0].fGreen;
      ...
      clr.fGreen =
          (m_pClr[2].fGreen - m_pClr[1].fGreen) * hs + m_pClr[1].fGreen;
...
void SmtTerrain::CreateNormal() {
  ...
  // new mem
  Vector4 *pNormals = new Vector4[m_p3DSurf->get_point_count()];

  // calculator normal
  for (int i = 0; i < m_p3DSurf->get_triangle_count(); i++) {
    Smt3DTriangle tri = m_p3DSurf->get_triangle(i);
    OGRPoint point1 = m_p3DSurf->get_point(tri.a);
    OGRPoint point2 = m_p3DSurf->get_point(tri.b);
    OGRPoint point3 = m_p3DSurf->get_point(tri.c);
