...
namespace render
{
	...
	void SmtGdiRenderThread::suspend()
	{
		m_suspend = true;
	}
	...
	int SmtGdiRenderThread::RenderMap(const SmtMap *pMap,int x,int y,int w,int h,int op)
	{
		...
		if (pMap != NULL)
		{
			// Do not bind leftover RHI to this GDI HWND — sharing the window
			// with D3D/Vulkan has caused STATUS_FATAL_APP_EXIT (0xC000041D).
			// Vector paint uses GDI RenderLayer below.
