...
SmtGdiRenderDevice::SmtGdiRenderDevice(HINSTANCE hInst)
  ...
  m_pRenderThread =
      new SmtGdiRenderThread(hInst, m_virViewport1, m_virViewport2);
}
...
int SmtGdiRenderDevice::Resize(int orgx, int orgy, int cx, int cy) {
  ...
  if (SMT_ERR_NONE == m_smtRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
      ...
      SMT_ERR_NONE == m_smtDynamicRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
                                                       m_Viewport.m_fVHeight) &&
      SMT_ERR_NONE == m_smtMapRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
                                                   m_Viewport.m_fVHeight) &&
      SMT_ERR_NONE == m_smtQuickRenderBuf.SetBufSize(m_Viewport.m_fVWidth,
                                                     m_Viewport.m_fVHeight)) {
    if (SMT_ERR_NONE ==
        ...
        SMT_ERR_NONE == m_smtQuickRenderBuf.SwapBuf(
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                            m_Viewport.m_fVWidth, m_Viewport.m_fVHeight,
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY) &&
        SMT_ERR_NONE == m_smtDynamicRenderBuf.SwapBuf(
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                            m_Viewport.m_fVWidth, m_Viewport.m_fVHeight,
                            m_Viewport.m_fVOX, m_Viewport.m_fVOY) &&
...
int SmtGdiRenderDevice::Refresh() {
  ...
  m_smtDynamicRenderBuf.SwapBuf(
      ...
      m_virViewport1.m_fVWidth, m_virViewport1.m_fVHeight,
      m_virViewport2.m_fVOX, m_virViewport2.m_fVOY, m_virViewport2.m_fVWidth,
      m_virViewport2.m_fVHeight, BLT_TRANSPARENT,
      SRCCOPY /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);

  m_smtQuickRenderBuf.SwapBuf(
      m_smtRenderBuf, m_virViewport1.m_fVOX, m_virViewport1.m_fVOY,
      m_virViewport1.m_fVWidth, m_virViewport1.m_fVHeight,
      m_virViewport2.m_fVOX, m_virViewport2.m_fVOY, m_virViewport2.m_fVWidth,
      m_virViewport2.m_fVHeight, BLT_TRANSPARENT,
...
int SmtGdiRenderDevice::RefreshDirectly(const SmtMap *pSmtMap, lRect rect,
  ...
  if (pSmtMap) {
    if (!bRealTime)
      return ReRenderMapByProxy(pSmtMap, m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                                m_Viewport.m_fVWidth, m_Viewport.m_fVHeight);
    else
      return ReRenderMapRealTime(pSmtMap, m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                                 m_Viewport.m_fVWidth, m_Viewport.m_fVHeight);
  }
...
int SmtGdiRenderDevice::PreviewZoomScale(lPoint orgPoint, float fscale) {
  // virViewport1/2 are references shared with the render thread. Mutating
  // them while RenderMap runs races and has crashed 2D wheel zoom.
  if (m_pRenderThread) {
    while (m_pRenderThread->IsRendering()) {
      Sleep(0);
    }
  }
  ...
  // TransparentBlt rejects non-positive extents; keep a 1px floor.
  if (m_virViewport1.m_fVWidth < 1.f) {
    m_virViewport1.m_fVWidth = 1.f;
  }
  if (m_virViewport1.m_fVHeight < 1.f) {
    m_virViewport1.m_fVHeight = 1.f;
  }
  if (m_virViewport2.m_fVWidth < 1.f) {
    m_virViewport2.m_fVWidth = 1.f;
  }
  if (m_virViewport2.m_fVHeight < 1.f) {
    m_virViewport2.m_fVHeight = 1.f;
  }
...
int SmtGdiRenderDevice::ScheduleDelayedRedraw(const SmtMap *pMap) {
  if (!pMap || !m_pRenderThread) {
    return SMT_ERR_INVALID_PARAM;
  }
  SmtRenderContex smtRC(m_Viewport, m_Windowport, m_fblc, pMap,
                        m_Viewport.m_fVOX, m_Viewport.m_fVOY,
                        m_Viewport.m_fVWidth, m_Viewport.m_fVHeight,
                        R2_COPYPEN);
...
int SmtGdiRenderDevice::Timer() {
  if (m_bRedraw) {
    ...
    if (dbfElapse > C_fDELAY) {  // delayed redraw
      m_bRedraw = false;
      m_pRenderThread->resume();  // wake map worker
    }
  }
...
int SmtGdiRenderDevice::BeginRender(eRDBufferLayer eMRDBufLyr, bool bClear,
  ...
  switch (eMRDBufLyr) {
    ...
    case MRD_BL_DYNAMIC: {
      if (bClear)
        m_smtDynamicRenderBuf.ClearBuf(
            m_Viewport.m_fVOX, m_Viewport.m_fVOY, m_Viewport.m_fVWidth,
            m_Viewport.m_fVHeight /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
  ...
  if (m_bCurUseStyle) {
    ...
    if (format & ST_BrushDesc) {
      ...
      if (brush.brushTp == SmtBrushDesc::BT_Hatch) {
        ...
        m_hBrush = CreateSolidBrush(fill);
...
int SmtGdiRenderDevice::ReRenderMapByProxy(const SmtMap *pMap, int x, int y,
  ...
  m_smtDynamicRenderBuf.ClearBuf(x, y, w,
                                 h /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
  m_smtQuickRenderBuf.ClearBuf(x, y, w,
                               h /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
...
int SmtGdiRenderDevice::ReRenderMapRealTime(const SmtMap *pMap, int x, int y,
  ...
  m_smtMapRenderBuf.ClearBuf(x, y, w,
                             h /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
  m_smtDynamicRenderBuf.ClearBuf(x, y, w,
                                 h /*,(COLORREF)::GetSysColor(COLOR_WINDOW)*/);
  m_smtQuickRenderBuf.ClearBuf(x, y, w,
...
int SmtGdiRenderDevice::RenderMap(const SmtMap *pMap, int op) {
  ...
  m_carto2d->reset(m_fblc, static_cast<int>(m_Viewport.m_fVWidth),
...
int SmtGdiRenderDevice::RenderFeature(OGRFeature *pFeature, int op) {
  ...
  auto field = [pFeature](const char *key) -> const char * {
    if (!pFeature || !key) {
      return "";
    }
    const int i = pFeature->GetFieldIndex(key);
    if (i < 0) {
      return "";
    }
...
int SmtGdiRenderDevice::DrawPoint(const SmtStyle *pStyle,
  ...
  if (!pStyle) {
    ...
    if (!m_carto2d->try_keep_point(static_cast<int>(lX), static_cast<int>(lY))) {
      return SMT_ERR_NONE;
    }
    draw_point_disc(m_hCurDC, lX, lY, carto2d_point_radius(m_fblc));
    return SMT_ERR_NONE;
  }
  ulong format = pStyle->get_style_type();
  if (m_nFeatureType == SmtFeatureType::SmtFtAnno) {
...
int SmtGdiRenderDevice::DrawAnno(const char *szAnno, float fangel,
  ...
  const MapCartoBox box =
      carto2d_label_box(static_cast<int>(x), static_cast<int>(y), szAnno, px_h,
                        m_labelPriority);
...
int SmtGdiRenderDevice::DrawImage(const char *szImageBuf, int nImageBufSize,
  ...
  switch (eMRDBufLyr) {
    ...
    case MRD_BL_DYNAMIC: {
      lRtn = m_smtDynamicRenderBuf.DrawImage(szImageBuf, nImageBufSize,
                                             lCodeType, lrt.lb.x, lrt.rt.y,
                                             lrt.width(), lrt.height());
    } break;

    case MRD_BL_QUICK: {
      lRtn = m_smtQuickRenderBuf.DrawImage(szImageBuf, nImageBufSize, lCodeType,
                                           lrt.lb.x, lrt.rt.y, lrt.width(),
                                           lrt.height());
    } break;
  }
...
int SmtGdiRenderDevice::StrethImage(const char *szImageBuf, int nImageBufSize,
  ...
  switch (eMRDBufLyr) {
    ...
    case MRD_BL_DYNAMIC: {
      lRtn = m_smtDynamicRenderBuf.StrethImage(szImageBuf, nImageBufSize,
                                               lCodeType, lrt.lb.x, lrt.rt.y,
                                               lrt.width(), lrt.height());
    } break;

    case MRD_BL_QUICK: {
      lRtn = m_smtQuickRenderBuf.StrethImage(szImageBuf, nImageBufSize,
                                             lCodeType, lrt.lb.x, lrt.rt.y,
                                             lrt.width(), lrt.height());
    } break;
  }
...
int SmtGdiRenderDevice::Save2ImageBuf(char *&szImageBuf, long &lImageBufSize,
  ...
  switch (eMRDBufLyr) {
    ...
    case MRD_BL_DYNAMIC: {
      lRtn = m_smtDynamicRenderBuf.Save2ImageBuf(szImageBuf, lImageBufSize,
                                                 lCodeType, bBgTransparent);
    } break;
    case MRD_BL_QUICK: {
      lRtn = m_smtQuickRenderBuf.Save2ImageBuf(szImageBuf, lImageBufSize,
                                               lCodeType, bBgTransparent);
    } break;
  }
