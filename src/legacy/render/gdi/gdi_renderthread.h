...
struct SmtRenderContex {
  ...
  SmtRenderContex(Viewport vp, Windowport wp, float _fblc, const SmtMap *_pMap,
                  int _x, int _y, int _w, int _h, int _op = R2_COPYPEN)
      : viewport(vp),
        windowport(wp),
        fblc(_fblc),
...
class SmtGdiRenderThread {
 public:
  SmtGdiRenderThread(HINSTANCE hInst, Viewport &virViewport1,
 ...
 public:
  ...
  int SetRenderContex(SmtRenderContex &smtRC) {
    if (!IsRendering()) m_smtRC = smtRC;
    return SMT_ERR_NONE;
  }
