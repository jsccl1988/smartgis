...
int SmtGdiRenderThread::RenderLayer(const SmtTileLayer *pLayer, int op) {
  ...
  while (!pLayer->IsEnd()) {
    ...
    if (NULL != pTile && pTile->bVisible) {
      ...
      StrethImage(pTile->pTileBuf, pTile->lTileBufSize, pTile->rtTileRect,
                  pTile->lImageCode);
    }
...
int SmtGdiRenderThread::RenderFeature(OGRFeature *pFeature, int op) {
  ...
  SmtStyle fallback;
  if (!pStyle) {
    gis::datasource::fill_default_draw_style(pFeature, &fallback, m_smtRC.fblc);
    pStyle = &fallback;
  }
  ...
  if (!(m_nFeatureType == SmtFeatureType::SmtFtAnno && m_szAnno[0])) {
    if (const char* picked = carto2d_label_text(field("anno"), field("name"),
                                                field("text"))) {
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiRenderThread::PrepareForDrawing(const SmtStyle *pStyle,
                                          int nDrawMode) {
  ...
  if (m_bCurUseStyle) {
    ...
    if (format & ST_PenDesc) {
      ...
      const int px =
          road_class > 0
              ? carto2d_road_width_px(m_smtRC.fblc, road_class)
              : carto2d_stroke_px_kind(m_smtRC.fblc, m_isRiver, false);
    ...
    if (format & ST_AnnoDesc) {
      ...
      m_hFont =
          CreateFont(anno.fHeight * m_smtRC.fblc, anno.fWidth * m_smtRC.fblc,
...
int SmtGdiRenderThread::RenderGeometry(const OGRGeometry *pGeom,
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiRenderThread::DrawMultiLineString(
...
int SmtGdiRenderThread::DrawMultiPoint(const SmtStyle *pStyle,
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiRenderThread::DrawPoint(const SmtStyle *pStyle,
  ...
  if (!pStyle) {
    ...
    draw_point_disc(m_hCurDC, lX, lY, carto2d_point_radius(m_smtRC.fblc));
    return SMT_ERR_NONE;
  }
  ulong format = pStyle->get_style_type();
  if (m_nFeatureType == SmtFeatureType::SmtFtAnno) {
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiRenderThread::DrawAnno(const char *szAnno, float fangel,
  ...
  const int px_h = carto2d_label_px(m_labelPriority, m_smtRC.fblc);
...
int SmtGdiRenderThread::DrawSymbol(HICON hIcon, long lHeight, long lWidth,
                                   const OGRPoint *pPoint) {
  ...
  //::DrawState(
  //: m_hCurDC,NULL,NULL,(LPARAM)hIcon,0,pt.x-lWidth/2,pt.y+lHeight/2,lWidth,lHeight,
  //: DSS_NORMAL | DST_ICON);
...
int SmtGdiRenderThread::DrawLineString(const OGRLineString *pLinestring) {
...

  {
    ...
    if (gfx.ok()) {
      if (m_roadClass > 0) {
        const int fill_w = carto2d_road_width_px(m_smtRC.fblc, m_roadClass);
...
//////////////////////////////////////////////////////////////////////////
int SmtGdiRenderThread::DrawEllipse(float left, float top, float right,
...
int SmtGdiRenderThread::DrawLine(const fPoint &ptA, const fPoint &ptB,
                                 bool bDP) {
...
int SmtGdiRenderThread::DrawText(const char *szAnno, float fangel,
                                 float fCHeight, float fCWidth, float fCSpace,
...
int SmtGdiRenderThread::DrawImage(const char *szImageBuf, int nImageBufSize,
...
int SmtGdiRenderThread::StrethImage(const char *szImageBuf, int nImageBufSize,
                                    const fRect &frect, long lCodeType) {
  lRect lrt;
  LRectToDRect(frect, lrt);
