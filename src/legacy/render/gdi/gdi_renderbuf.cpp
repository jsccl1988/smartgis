...
namespace render {
SmtRenderBuf::SmtRenderBuf(HWND hWnd)
    : m_hWnd(hWnd),
      m_hPaintDC(NULL),
      m_hOldPaintBuf(NULL),
      m_hPaintBuf(NULL),
...
long SmtRenderBuf::SetBufSize(int cx, int cy) {
  ...
  m_nBufHeight = cy;
  m_hPaintBuf =
      hDC ? CreateCompatibleBitmap(hDC, m_nBufWidth, m_nBufHeight) : NULL;
  if (hDC) {
...
long SmtRenderBuf::ClearBuf(int x, int y, int w, int h, COLORREF clr) {
  ...
  m_cslock.lock();
#endif
  // GDI FillRect requires top < bottom in MM_TEXT. The old
  // bottom=y / top=y+h order left the rect empty, so "clears" were no-ops
  // and rubber-band COPYPEN strokes stacked in MRD_BL_QUICK.
  RECT rect;
  rect.left = x;
  rect.top = y;
  rect.right = x + w;
...
long SmtRenderBuf::SwapBuf(int destOrgx, int destOrgy, int destW, int destH,
...
long SmtRenderBuf::SwapBuf(int destOrgx, int destOrgy, int destW, int destH,
  ...
  switch (type) {
    ...
    case BLT_TRANSPARENT:
      // Source size must follow srcW/srcH (zoom preview uses different
      // virViewport extents). Passing destW/destH twice mis-stretched and
      // could read past the map buffer during wheel zoom.
      bRet = ::TransparentBlt(hDC, destOrgx, destOrgy, destW, destH, hSrcDC,
                              srcOrgx, srcOrgy, srcW, srcH, clr);
      break;
  }
...
long SmtRenderBuf::SwapBuf(SmtRenderBuf &rbTarget, int destOrgx, int destOrgy,
...
//////////////////////////////////////////////////////////////////////////
long SmtRenderBuf::DrawImage(const char *szImageBuf, int nImageBufSize,
...
long SmtRenderBuf::StrethImage(const char *szImageBuf, int nImageBufSize,
...
long SmtRenderBuf::Save2ImageBuf(char *&szImageBuf, long &lImageBufSize,
...
//////////////////////////////////////////////////////////////////////////
long SmtRenderBuf::Save2Image(HBITMAP hBitMap, const char *szFilePath,
                              bool bBgTransparent) {
  ...
  if (image.CreateFromHBITMAP(hBitMap)) {
    ...
    if (image.Save(szFilePath, get_image_type_by_file_ext(szFilePath))) {
      return SMT_ERR_NONE;
    }
  }
...
long SmtRenderBuf::Save2ImageBuf(HBITMAP hBitMap, char *&szImageBuf,
  ...
  if (image.CreateFromHBITMAP(hBitMap)) {
    ...
    if (image.Encode(pImageBuf, lSize, (uint32_t)lCodeType)) {
