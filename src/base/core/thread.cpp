...
namespace base
{
	...
	void SmtThread::suspend()
	{
		if (m_bStarted && !m_bDetached)
		{
			...
			set_state(SMT_THREAD_SUSPENDED);
		}
	}
	...
	bool SmtThread::kill()
	{
			...
			::CloseHandle(m_hThread))
		{
			...
			set_state(SMT_THREAD_TERMINATED);
	...
	void SmtThread::set_priority(SmtThreadPriority priority)
	{
		switch(priority)
		{
	...
	SmtThreadPriority SmtThread::get_priority()
	{
	...
	{
		SmtThread *worker = (SmtThread *)pThread;
		if (NULL != pThread)
		{
			worker->set_state(SMT_THREAD_RUNNING);
			worker->run(worker->m_pParam);
			worker->set_state(SMT_THREAD_FINISHED);
		}
