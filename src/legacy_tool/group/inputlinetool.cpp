...
namespace tool
{
	...
	int SmtInputLineTool::KeyDown(uint nChar, uint nRepCnt, uint nFlags)
	{
	...
	void SmtInputLineTool::apply_draft(const tool::Draft& draft)
	{
		...
		SMT_SAFE_DELETE(m_pGeom);
		if (m_appendType == LT_LinearRing || draft.kind == tool::DraftKind::kPolygon)
		{
