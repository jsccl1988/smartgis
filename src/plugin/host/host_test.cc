...
#include "plugin/host/official_key.h"
...
int main() {
  ...
  {
    namespace fs = std::filesystem;
    // Prefer a repo-local path under out/ (writable in this build tree).
    const fs::path tmp = fs::path("out") / "smartgis_plugin_state_test";
    std::error_code ec;
    fs::remove_all(tmp, ec);
