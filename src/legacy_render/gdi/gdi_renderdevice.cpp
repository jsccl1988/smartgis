...
#include "legacy_render/gdi/gdi_renderdevice.h"
...
namespace render
{
	...
	int SmtGdiRenderDevice::Init(HWND hWnd,const char * logname)
	{
	   ...
	   pLog->log_message("Init Gdi SmtRenderDevice ok!");
	...
	int SmtGdiRenderDevice::Destroy(void)
	{
		...
		if (pLog != NULL)
			pLog->log_message("Destroy Gdi SmtRenderDevice ok!");
	...
	int SmtGdiRenderDevice::Release(void)
	{
	   ...
	   SmtLog *pLog = pLogMgr->get_log(m_strLogName.c_str());
	   if (pLog != NULL)
		   pLog->log_message("Release Gdi SmtRenderDevice ok!");
