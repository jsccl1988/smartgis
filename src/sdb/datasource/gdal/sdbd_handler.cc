...
#include "sdb/datasource/gdal/sdbd_json.h"
...
std::vector<LayerInfo> SdbdHandler::catalog() const {
  ...
  if (dataset_->GetRasterCount() > 0) {
    LayerInfo ras;
    ras.name = dataset_->GetDescription() ? dataset_->GetDescription() : "raster";
...
std::vector<std::int64_t> SdbdHandler::query_hits(const OpenRequest& req) const {
...
FeatureSet SdbdHandler::features_of(const std::string& layer,
                                    const std::string& crs, int offset,
...
bool SdbdHandler::open_dataset(const std::string& body, int* status,
  ...
  dataset_ = static_cast<GDALDataset*>(GDALOpenEx(
      name.c_str(), GDAL_OF_VECTOR | GDAL_OF_RASTER | GDAL_OF_UPDATE, nullptr,
      nullptr, nullptr));
  if (!dataset_) {
...
bool SdbdHandler::create_layer(const std::string& body, int* status,
  ...
  if (!info.crs.empty() && srs.SetFromUserInput(info.crs.c_str()) == OGRERR_NONE) {
    srs_ptr = &srs;
  }
...
void SdbdHandler::handle(const std::string& method, const std::string& raw,
  ...
  if (path == "/sdbd/api/v1/recordset/open" ||
    ...
    std::ostringstream o;
    o << "{\"ok\":true,\"handle\":\"" << handle << "\",\"schema\":"
      << layer_info_to_json(info) << ",\"matched\":" << hits.size() << '}';
    write_ok(o.str(), status, response);
    return;
  }
  ...
  if (path == "/sdbd/api/v1/recordset/fetch") {
    ...
    write_ok(feature_set_to_json(features_of(it->second.layer, it->second.crs,
                                             offset, slice)),
             status, response);
    return;
  }
