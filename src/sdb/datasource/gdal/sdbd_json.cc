...
std::string open_request_to_json(const OpenRequest& req) {
  ...
  o << "],\"bbox\":{\"min_x\":" << req.bbox.min_x << ",\"min_y\":"
    ...
    << req.bbox.max_y << "},\"predicate\":\"" << json_escape(req.predicate)
    << '"';
  if (!req.crs.empty()) {
...
std::string layer_info_to_json(const LayerInfo& info) {
  ...
  o << "{\"name\":\"" << json_escape(info.name) << "\",\"crs\":\""
    ...
    << json_escape(info.geom_column) << "\",\"extent\":{\"min_x\":"
...
std::string feature_set_to_json(const FeatureSet& set) {
  ...
  o << "{\"layer\":\"" << json_escape(set.layer) << "\",\"crs\":\""
    << json_escape(set.crs) << "\",\"count\":" << set.count << ",\"offset\":"
    << set.offset << ",\"features\":[";
...
bool parse_layer_info(const std::string& text, LayerInfo* out, std::string* err) {
  Json json;
