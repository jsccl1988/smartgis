...
bool compare_values(FilterOp op, const std::string& left, const std::string& right) {
  bool lok = false;
  bool rok = false;
...
std::vector<const StyleLayer*> select_layers(const StyleDocument& doc,
  ...
  for (const auto& layer : doc.layers) {
    ...
    // Non-empty request: require exact source-layer (blank layer field does not match).
...
bool resolve(const StyleDocument& doc,
