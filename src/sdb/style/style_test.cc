...
int main() {
  ...
  expect(approx_eq(paint.line_dasharray[0], 2.f) && approx_eq(paint.line_dasharray[1], 4.f),
         "dasharray values");
  ...
  expect(eval_expression("[\"literal\",3.5]", expr_attrs, 10.0, &lit_ev),
         "eval literal");
  expect(approx_eq(static_cast<float>(lit_ev.as_number()), 3.5f), "literal value");
  ExprValue cmp_ev;
