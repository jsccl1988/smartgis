...
double expr_as_number(const ExprValue& v, bool* ok) {
  return v.as_number(ok);
}

std::string expr_as_string(const ExprValue& v) {
  return v.as_string();
}

bool compare_expr(const std::string& op,
...
bool eval_json(const JsonValue& v,
               const AttrMap& attrs,
               double zoom,
               ExprValue* out) {
  if (!out) {
...
bool eval_expression(const std::string& json,
                     const AttrMap& attrs,
                     double zoom,
                     ExprValue* out) {
  if (!out || json.empty()) {
