...
int main() {
  using sdb::tile::format_xyz_url;
  using sdb::tile::tile_world_rect;
  using sdb::tile::tiles_for_viewport;
  using sdb::tile::TileCoord;
  using sdb::tile::TileImage;
  using sdb::tile::TileProvider;
  using sdb::tile::Viewport;

  {
    const std::string url = format_xyz_url(
        "http://tile.example/{z}/{x}/{y}.png", 3, 1, 2);
    expect(url == "http://tile.example/3/1/2.png", "format_xyz_url z/x/y");
    const std::string sub = format_xyz_url(
        "http://{s}.tile.example/{z}/{x}/{y}.png", 0, 0, 0);
    expect(sub == "http://a.tile.example/0/0/0.png", "format_xyz_url subdomain");
  }
...

  {
    ...
    vp.max_y = wr.rt.y;
    vp.z = 2;
    auto* layer = new sdb::tile::ProviderTileLayer(provider);
    ...
    sdb::scene::TessMesh mesh;
    expect(sdb::scene::tessellate_tile_layer(layer, mesh), "tessellate");
    expect(mesh.has_image, "tessellate has_image");

    sdb::MapLayer map_layer = sdb::tile::make_map_layer(provider);
    expect(map_layer.layer_type() == sdb::LYR_TITLE, "MapLayer kind=tile");
    expect(map_layer.leftover() != nullptr, "leftover ProviderTileLayer");
    expect(map_layer.ogr() == nullptr, "tile MapLayer has no OGR");

    sdb::MapLayer xyz =
        sdb::tile::make_xyz_map_layer("http://tiles.local/{z}/{x}/{y}.png");
    expect(xyz.layer_type() == sdb::LYR_TITLE, "make_xyz_map_layer kind=tile");
    expect(xyz.leftover() != nullptr, "make_xyz_map_layer leftover");
    sdb::MapLayer bad = sdb::tile::make_xyz_map_layer("http://bad/{z}");
  ...
  // WMTS Capabilities fixture (no network).
  {
    ...
    std::string tmpl;
    std::string err;
    expect(sdb::tile::parse_wmts_capabilities(k_caps, &tmpl, &err),
           "parse wmts caps");
    ...
    sdb::MapLayer layer =
        sdb::tile::make_wmts_map_layer_from_capabilities(k_caps);
    expect(layer.leftover() != nullptr, "make_wmts from caps");

    std::string norm;
    expect(sdb::tile::normalize_wmts_url_template(
