...
// Lightweight MapLibre-aligned source description (raster subset + typed stubs).
// Corresponds to one entry under Style JSON `sources` (id is the map key).
// StyleDocument today stores layer.source as a string only; this type owns the
// data-plane fields (tiles / tileSize) needed to open TileProvider.
