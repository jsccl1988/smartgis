...
int main() {
  using sdb::scene::LineCap;
  using sdb::scene::LineJoin;
  using sdb::scene::LineTessOptions;
  using sdb::scene::TessMesh;

  // Empty / invalid geometry → false.
  TessMesh empty_mesh;
  expect(!sdb::scene::tessellate_line(nullptr, empty_mesh), "null line");
  OGRLineString empty_line;
  expect(!sdb::scene::tessellate_line(&empty_line, empty_mesh), "0-pt line");
  empty_line.addPoint(1, 1);
  expect(!sdb::scene::tessellate_line(&empty_line, empty_mesh), "1-pt line");

  LineTessOptions opts;
  opts.half_width = 1.0;
  expect(!sdb::scene::tessellate_line(nullptr, opts, empty_mesh),
         "styled null");
  expect(!sdb::scene::tessellate_polyline(nullptr, 0, 2, opts, empty_mesh),
         "polyline null");
  ...
  expect(sdb::scene::tessellate_line(&seg, opts, solid), "solid segment");
  ...
  TessMesh from_px;
  expect(sdb::scene::tessellate_line(&seg, px, from_px), "pixel width");
  expect(from_px.indices.size() == solid.indices.size(),
         "pixel path index count");
  expect(sdb::scene::line_half_width_world(2.0, 0.5) == 0.5, "half world");
  expect(sdb::scene::line_half_width_from_envelope(2.0, 100.0, 200.0) == 0.5,
         "half from envelope");
  ...
  expect(sdb::scene::tessellate_line(&corner, opts, corner_mesh), "corner");
  ...
  TessMesh dash_mesh;
  expect(sdb::scene::tessellate_line(&seg, dashed, dash_mesh), "dash line");
  ...
  TessMesh round_mesh;
  expect(sdb::scene::tessellate_line(&seg, round_caps, round_mesh),
         "round caps");
  ...
  TessMesh legacy;
  expect(sdb::scene::tessellate_line(&seg, legacy), "legacy line");
