...
double vec_cross(Vec2 a, Vec2 b) {
...
// Legacy per-segment ribbon (fixed half-width 0.05). Kept for tessellate_geometry.
...
void emit_end_caps(const std::vector<PolyPt>& pts, double hw, LineCap cap,
  ...
  const Vec2 d0 =
      vec_normalize({pts[1].x - pts[0].x, pts[1].y - pts[0].y});
  const Vec2 d1 = vec_normalize(
