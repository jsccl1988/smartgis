...
namespace app
{
	...
	bool SmtApp::Destory()
	{
		...
		//////////////////////////////////////////////////////////////////////////
		SmtPluginManager::get_singleton_ptr()->destroy_instance();
		...
		SmtLogManager::get_singleton_ptr()->destroy_instance();
	...
	bool SmtApp::InitLogMgr(void)
	{
		...
		split_file_name(fullpath,path,fileName,title,ext);
		...
		if (pLogMgr)
		{
			pLogMgr->set_log_dir(strLogDir.c_str());

			if (pLogMgr->set_default_log("SmtDefault"))
			{
	...
	bool SmtApp::InitSmtAuxModules(void)
	{
		...
		if (pPluginMgr)
		{
			...
			pPluginMgr->load_all_plugin(strPluginPath.c_str());
		}
