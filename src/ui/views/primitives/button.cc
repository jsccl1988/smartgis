...
// Matches paint_self text inset (8px x, 6px y each side).
constexpr int kPadX = 16;
constexpr int kPadY = 12;
constexpr int kMinHeight = 28;

Size preferred_for_text(const std::string& text) {
  const Size ink = measure_text_utf8(text);
  const int h = ink.height + kPadY;
  return {ink.width + kPadX, h > kMinHeight ? h : kMinHeight};
}

}  // namespace

Button::Button(std::string text) : text_(std::move(text)) {
  set_preferred_size(preferred_for_text(text_));
  set_focusable(true);
}
