...
namespace Smt_XView
{
...
#ifdef _DEBUG
	void Smt2DXView::AssertValid() const
	{
		SmtXView::AssertValid();
	}

#ifndef _WIN32_WCE
	void Smt2DXView::Dump(CDumpContext& dc) const
	{
		SmtXView::Dump(dc);
	}
#endif
	...
	bool Smt2DXView::CreateTools(void)
	{
		...
		SmtGroupToolFactory::CreateGroupTool(m_pViewCtrlTool,GroupToolType::GTT_ViewControl);
		...
		if (m_pViewCtrlTool->Init(m_pRenderDevice,m_pSmtOperMap,m_hWnd) != SMT_ERR_NONE)
			return false;

		m_pSelectTool->SetToolStyleName(pSysMgr->GetSysStyleConfig().szAuxStyle);
		if (m_pSelectTool->Init(m_pRenderDevice,m_pSmtOperMap,m_hWnd) != SMT_ERR_NONE)
			return false;

		m_pFlashTool->SetToolStyleName(pSysMgr->GetSysStyleConfig().szAuxStyle);
		if (m_pFlashTool->Init(m_pRenderDevice,m_pSmtOperMap,m_hWnd) != SMT_ERR_NONE)
			return false;
