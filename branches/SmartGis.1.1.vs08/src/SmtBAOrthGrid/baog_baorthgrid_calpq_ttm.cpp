...
{
	void  SmtBAOrthGrid::CalP3(Matrix2D<double> *pP)
	{
		int i ,j;
		dbfPoint Pw,Pe,P;
		...
		for (i = 1;i < m_nX - 1 ; i++)
		{
			...
			if(xksks < 0.001) xksks = 0;
			if(yksks < 0.001) yksks = 0;
	...
	void  SmtBAOrthGrid::CalQ3(Matrix2D<double> *pQ)
	{
		int i ,j;
		dbfPoint Ps,Pn,P;
		...
		for (j = 1; j < m_nY - 1; j++)
		{
			...
			yat = (Pn.y - Ps.y)/2.;
			yatat = Pn.y -2*P.y+ Ps.y;
			if(xatat < 0.001) xatat = 0;
			if(yatat < 0.001) yatat = 0;
			arfa = xat*xat + yat*yat;
			f = -(xat*xatat+yat*yatat)/arfa;
			pQ->SetElement(f,m_nX -1,j);
		}
